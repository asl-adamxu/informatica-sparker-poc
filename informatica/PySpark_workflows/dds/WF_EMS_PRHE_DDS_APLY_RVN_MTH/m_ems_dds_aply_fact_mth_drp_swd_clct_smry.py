'''
* Script - M_EMS_DDS_APLY_FACT_MTH_DRP_SWD_CLCT_SMRY.py
* @date    2026-07-02
* @author Auto-generated by ASL Informatica XML to PySpark Converter
* @version 1.0
* @par Remarks
*   This code is auto-generated. Please review the business rules before productionizing.
'''

import env.runtime_lib as lib
from pyspark.sql import DataFrame
# Save builtins before pyspark.sql.functions shadows max/min with column versions
_builtin_max = max
_builtin_min = min
from pyspark.sql.functions import *
from pyspark.sql.types import *

# =============================================================================
# MAPPING LOGIC
# =============================================================================

def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None) -> bool:
    """
    Execute the M_EMS_DDS_APLY_FACT_MTH_DRP_SWD_CLCT_SMRY mapping transformations.

    All initialization (logging, config, spark session, connections) happens
    lazily inside this function, so importing the module has no side effects.
    
    Args:
        ctx: Optional SparkContext for session and DataFrame registry
        metrics: Optional metrics tracker (or NullMetrics if not provided)
        job_params: Optional dict of job parameters loaded by workflow
    
    Returns:
        bool: True if successful
    """
    lib.init_logger()
    logger = lib.get_logger()
    logger.propagate = False  # mapping detail stays in mapping log only
    logger.info("Start to run mapping M_EMS_DDS_APLY_FACT_MTH_DRP_SWD_CLCT_SMRY")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_EMS_DDS_APLY_FACT_MTH_DRP_SWD_CLCT_SMRY", config)
    metrics = metrics or lib.NullMetrics()
    metrics.start()

    conn_oracle = lib.get_db_config(config, "oracle-defaults")
    conn_source = lib.get_db_config(config, "DPA")
    conn_target = lib.get_db_config(config, "DPA")

    v_REC_RLS_IND = ""
    # Load mapping variables from job_params or UTL_JOB_PARAM file
    if job_params:
        for _var, _val in job_params.items():
            _clean = _var.replace("$", "")
            if _clean in [ "v_REC_RLS_IND", ]:
                if _clean == "v_REC_RLS_IND":
                    v_REC_RLS_IND = _val
                logger.info("Loaded %s=%s from job_params", _var, _val)
    else:
        try:
            _param_obj = objects.get("UTL_JOB_PARAM", {})
            if isinstance(_param_obj, dict):
                _param_path = lib._resolve_path(_param_obj.get('path', '/tmp/UTL_JOB_PARAM'))
            with open(_param_path, "r") as _f:
                for _line in _f:
                    _line = _line.strip()
                    for _var in [ "$$v_REC_RLS_IND", ]:
                        if _line.startswith(_var + "="):
                            _val = _line.split("=", 1)[1]
                            _clean = _var.replace("$", "")
                            if _clean == "v_REC_RLS_IND":
                                v_REC_RLS_IND = _val
                            logger.info("Loaded %s=%s from %s", _var, _val, _param_path)
        except Exception:
            logger.warning("UTL_JOB_PARAM not found, using default values")
    
    try:
        logger.info("Step: read_DPA_FACT_MTH_DRP_SWD_CLCT_SMRY")
        # Reading Data From Source - read_DPA_FACT_MTH_DRP_SWD_CLCT_SMRY
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_src_1 = lib.read_sql(spark, _conn, table="DPA_FACT_MTH_DRP_SWD_CLCT_SMRY")
        
        logger.info("Step: apply_SQ_SP_DELETE")
        # Source Qualifier: apply_SQ_SP_DELETE
        df_sq_2 = df_src_1
        # Select only SQ output ports (matches Informatica behavior)
        df_sq_2 = df_sq_2.select("TIME_DMNS_KEY")
        ctx.register_df("df_sq_2", df_sq_2)
        
        logger.info("Step: apply_SQ_DPA_FACT_MTH_DRP_SWD_CLCT_SMRY")
        # Source Qualifier: apply_SQ_DPA_FACT_MTH_DRP_SWD_CLCT_SMRY
        df_sq_3 = df_src_1
        # Select only SQ output ports (matches Informatica behavior)
        df_sq_3 = df_sq_3.select("TIME_DMNS_KEY", "RVN_TXN_MODE_DMNS_KEY", "EST_SCD_KEY", "DRP_ACTL_TXN_PYMT_ITEM_AMT", "DRP_ACTL_TXN_CNT", "DRP_ACTL_TXN_TNCY_CNT", "DRP_PRLM_FILE_TNCY_CNT", "DRP_OVER_PAY_TNCY_CNT", "DRP_EXACT_PAY_TNCY_CNT", "DRP_UND_PAY_TNCY_CNT", "DRP_CMLT_ARR_AMT", "DRP_UND_PAY_CMLT_ARR_AMT", "DRP_EXACT_PAY_ARR_TNCY_CNT", "DRP_UND_PAY_ARR_TNCY_CNT", "DRP_TNCY_CMLT_ADV_AMT", "DRP_ADV_TNCY_CNT", "DRP_HALF_RENT_TNCY_CNT", "DRP_THRD_QTR_RENT_TNCY_CNT", "DRP_NRML_RENT_TNCY_CNT", "DRP_EXTRA_RENT_TNCY_CNT", "DRP_DBL_RENT_TNCY_CNT", "DRP_MKT_RENT_TNCY_CNT", "DRP_CASE_CNT", "DRP_REJ_TXN_PYMT_ITEM_AMT", "DRP_REJ_TXN_CNT")
        ctx.register_df("df_sq_3", df_sq_3)
        
        logger.info("Step: apply_EXP_SET_DEL_INFO")
        # Expression: apply_EXP_SET_DEL_INFO
        df_exp_4 = df_sq_2
        df_exp_4 = df_exp_4.withColumn("TBL_NAME", expr("'DDS_FACT_MTH_DRP_SWD_CLCT_SMRY'"))
        df_exp_4 = df_exp_4.withColumn("RM_FLG", expr("'Y'"))
        _expr = """'$$v_REC_RLS_IND'"""
        _expr = _expr.replace("$$v_REC_RLS_IND", str(v_REC_RLS_IND))
        df_exp_4 = df_exp_4.withColumn("RSL_CTL_IND", expr(_expr))
        # Ensure any missing pass-through columns exist (no connector feeding them)
        for _col in ["TIME_DMNS_KEY"]:
            if _col not in df_exp_4.columns:
                df_exp_4 = df_exp_4.withColumn(_col, lit(None))
        # Select only mapping output ports (prevents column leakage)
        df_exp_4 = df_exp_4.select("TIME_DMNS_KEY", "TBL_NAME", "RM_FLG", "RSL_CTL_IND")
        ctx.register_df("df_exp_4", df_exp_4)
        
        logger.info("Step: apply_AGGTRANS")
        # Aggregator: apply_AGGTRANS
        df_agg_5 = df_sq_3.groupBy("TIME_DMNS_KEY", "EST_SCD_KEY")
        df_agg_5 = df_agg_5.agg(
            max("RVN_TXN_MODE_DMNS_KEY").alias("RVN_TXN_MODE_DMNS_KEY1"),
            sum("DRP_ACTL_TXN_PYMT_ITEM_AMT").alias("DRP_ACTL_TXN_PYMT_ITEM_AMT1"),
            sum("DRP_ACTL_TXN_CNT").alias("DRP_ACTL_TXN_CNT1"),
            sum("DRP_ACTL_TXN_TNCY_CNT").alias("DRP_ACTL_TXN_TNCY_CNT1"),
            sum("DRP_PRLM_FILE_TNCY_CNT").alias("DRP_PRLM_FILE_TNCY_CNT1"),
            sum("DRP_OVER_PAY_TNCY_CNT").alias("DRP_OVER_PAY_TNCY_CNT1"),
            sum("DRP_EXACT_PAY_TNCY_CNT").alias("DRP_EXACT_PAY_TNCY_CNT1"),
            sum("DRP_UND_PAY_TNCY_CNT").alias("DRP_UND_PAY_TNCY_CNT1"),
            sum("DRP_CMLT_ARR_AMT").alias("DRP_CMLT_ARR_AMT1"),
            sum("DRP_UND_PAY_CMLT_ARR_AMT").alias("DRP_UND_PAY_CMLT_ARR_AMT1"),
            sum("DRP_EXACT_PAY_ARR_TNCY_CNT").alias("DRP_EXACT_PAY_ARR_TNCY_CNT1"),
            sum("DRP_UND_PAY_ARR_TNCY_CNT").alias("DRP_UND_PAY_ARR_TNCY_CNT1"),
            sum("DRP_TNCY_CMLT_ADV_AMT").alias("DRP_TNCY_CMLT_ADV_AMT1"),
            sum("DRP_ADV_TNCY_CNT").alias("DRP_ADV_TNCY_CNT1"),
            sum("DRP_HALF_RENT_TNCY_CNT").alias("DRP_HALF_RENT_TNCY_CNT1"),
            sum("DRP_THRD_QTR_RENT_TNCY_CNT").alias("DRP_THRD_QTR_RENT_TNCY_CNT1"),
            sum("DRP_NRML_RENT_TNCY_CNT").alias("DRP_NRML_RENT_TNCY_CNT1"),
            sum("DRP_EXTRA_RENT_TNCY_CNT").alias("DRP_EXTRA_RENT_TNCY_CNT1"),
            sum("DRP_DBL_RENT_TNCY_CNT").alias("DRP_DBL_RENT_TNCY_CNT1"),
            sum("DRP_MKT_RENT_TNCY_CNT").alias("DRP_MKT_RENT_TNCY_CNT1"),
            sum("DRP_CASE_CNT").alias("DRP_CASE_CNT1"),
            sum("DRP_REJ_TXN_PYMT_ITEM_AMT").alias("DRP_REJ_TXN_PYMT_ITEM_AMT1"),
            sum("DRP_REJ_TXN_CNT").alias("DRP_REJ_TXN_CNT1")
        )
        ctx.register_df("df_agg_5", df_agg_5)
        
        logger.info("Step: apply_mplt_EXPTRANS")
        # Expression: apply_mplt_EXPTRANS
        df_mplt_expr_6 = df_exp_4
        df_mplt_expr_6 = df_mplt_expr_6.withColumn("RLS_CNTL_DMNS_TYPE_CODE", expr("substring(cast(TIME_DMNS_KEY as string),1,1)"))
        ctx.register_df("df_mplt_expr_6", df_mplt_expr_6)
        
        logger.info("Step: read_mplt_LKPTRANS")
        # Reading Data From Source - read_mplt_LKPTRANS
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "target")
        df_mplt_lkp_7 = lib.read_sql(spark, _conn, table="DDS_RLS_CNTL")
        
        logger.info("Step: apply_mplt_EXP_SP_DELETE")
        # Expression: apply_mplt_EXP_SP_DELETE
        df_mplt_expr_8 = df_exp_4
        df_mplt_expr_8 = df_mplt_expr_8.withColumn("FACT_TBL_NAME", expr("TBL_NAME"))
        # Execute stored procedure for each input value via JDBC
        _sp_conn = conn_oracle
        _input_vals = [row["TBL_NAME, MIN_TIME_DMNS_KEY, RM_FLG"] for row in df_exp_4.select("TBL_NAME, MIN_TIME_DMNS_KEY, RM_FLG").collect()]
        for _val in _input_vals:
            lib.execute_sql(spark, _sp_conn,
                "BEGIN SP_DELETE_DDS_FACT('" + _val + "'); END;")
        df_mplt_expr_8 = df_mplt_expr_8.withColumn("CALL_SP", lit("SUCCESS"))
        df_mplt_expr_8 = df_mplt_expr_8.withColumn("RLS_CNTL_BGN_DMNS_KEY", expr("CASE WHEN (RLS_CNTL_BGN_TIME_DMNS_KEY IS NULL) OR (MIN_TIME_DMNS_KEY < RLS_CNTL_BGN_TIME_DMNS_KEY) THEN MIN_TIME_DMNS_KEY ELSE RLS_CNTL_BGN_TIME_DMNS_KEY END"))
        df_mplt_expr_8 = df_mplt_expr_8.withColumn("RLS_CNTL_END_DMNS_KEY", expr("CASE WHEN (RLS_CNTL_END_TIME_DMNS_KEY IS NULL) OR (MAX_TIME_DMNS_KEY > RLS_CNTL_END_TIME_DMNS_KEY) THEN MAX_TIME_DMNS_KEY ELSE RLS_CNTL_END_TIME_DMNS_KEY END"))
        df_mplt_expr_8 = df_mplt_expr_8.withColumn("UPDATE_FLAG", expr("CASE WHEN (DDS_RLS_CNTL_TBL_NAME IS NULL) THEN DD_INSERT ELSE DD_UPDATE END"))
        ctx.register_df("df_mplt_expr_8", df_mplt_expr_8)
        
        logger.info("Step: apply_mplt_FILTRANS")
        # Filter: apply_mplt_FILTRANS
        df_mplt_fil_9 = df_mplt_expr_8.filter(expr("RSL_CTL_IND = '1'"))
        ctx.register_df("df_mplt_fil_9", df_mplt_fil_9)
        
        logger.info("Step: apply_MPLT_DDS_APPLY_DELETE_AFFECT_RECORD")
        # Expression: apply_MPLT_DDS_APPLY_DELETE_AFFECT_RECORD
        df_mplt_10 = df_mplt_expr_8
        ctx.register_df("df_mplt_10", df_mplt_10)
        
        logger.info("Step: write_DDS_FACT_MTH_DRP_SWD_CLCT_SMRY")
        # Write to Target: write_DDS_FACT_MTH_DRP_SWD_CLCT_SMRY
        df_write = df_agg_5
        # Cast columns to match target schema data types
        if "drp_actl_txn_cnt" in [c.lower() for c in df_write.columns]:
            for c in df_write.columns:
                if c.lower() == "drp_actl_txn_cnt":
                    df_write = df_write.withColumn(c,
                        when(col(c).cast(DecimalType(38,0)).isNotNull(),
                             col(c).cast(DecimalType(38,0)).cast(StringType()))
                        .otherwise(col(c).cast(StringType())))
        # Map source columns to target columns using connector field map (handles name mismatches)
        _field_map = {"DRP_ACTL_TXN_CNT": "DRP_ACTL_TXN_CNT1", "DRP_ACTL_TXN_PYMT_ITEM_AMT": "DRP_ACTL_TXN_PYMT_ITEM_AMT1", "DRP_ACTL_TXN_TNCY_CNT": "DRP_ACTL_TXN_TNCY_CNT1", "DRP_ADV_TNCY_CNT": "DRP_ADV_TNCY_CNT1", "DRP_CASE_CNT": "DRP_CASE_CNT1", "DRP_CMLT_ARR_AMT": "DRP_CMLT_ARR_AMT1", "DRP_DBL_RENT_TNCY_CNT": "DRP_DBL_RENT_TNCY_CNT1", "DRP_EXACT_PAY_ARR_TNCY_CNT": "DRP_EXACT_PAY_ARR_TNCY_CNT1", "DRP_EXACT_PAY_TNCY_CNT": "DRP_EXACT_PAY_TNCY_CNT1", "DRP_EXTRA_RENT_TNCY_CNT": "DRP_EXTRA_RENT_TNCY_CNT1", "DRP_HALF_RENT_TNCY_CNT": "DRP_HALF_RENT_TNCY_CNT1", "DRP_MKT_RENT_TNCY_CNT": "DRP_MKT_RENT_TNCY_CNT1", "DRP_NRML_RENT_TNCY_CNT": "DRP_NRML_RENT_TNCY_CNT1", "DRP_OVER_PAY_TNCY_CNT": "DRP_OVER_PAY_TNCY_CNT1", "DRP_PRLM_FILE_TNCY_CNT": "DRP_PRLM_FILE_TNCY_CNT1", "DRP_REJ_TXN_CNT": "DRP_REJ_TXN_CNT1", "DRP_REJ_TXN_PYMT_ITEM_AMT": "DRP_REJ_TXN_PYMT_ITEM_AMT1", "DRP_THRD_QTR_RENT_TNCY_CNT": "DRP_THRD_QTR_RENT_TNCY_CNT1", "DRP_TNCY_CMLT_ADV_AMT": "DRP_TNCY_CMLT_ADV_AMT1", "DRP_UND_PAY_ARR_TNCY_CNT": "DRP_UND_PAY_ARR_TNCY_CNT1", "DRP_UND_PAY_CMLT_ARR_AMT": "DRP_UND_PAY_CMLT_ARR_AMT1", "DRP_UND_PAY_TNCY_CNT": "DRP_UND_PAY_TNCY_CNT1", "EST_SCD_KEY": "EST_SCD_KEY", "RVN_TXN_MODE_DMNS_KEY": "RVN_TXN_MODE_DMNS_KEY1", "TIME_DMNS_KEY": "TIME_DMNS_KEY"}
        for _tgt_col, _src_col in _field_map.items():
            if _tgt_col not in df_write.columns and _src_col in df_write.columns:
                df_write = df_write.withColumnRenamed(_src_col, _tgt_col)
        # Select only target-defined columns (field_map already handled name alignment)
        _target_cols = ['TIME_DMNS_KEY', 'RVN_TXN_MODE_DMNS_KEY', 'EST_SCD_KEY', 'DRP_ACTL_TXN_PYMT_ITEM_AMT', 'DRP_ACTL_TXN_CNT', 'DRP_ACTL_TXN_TNCY_CNT', 'DRP_PRLM_FILE_TNCY_CNT', 'DRP_OVER_PAY_TNCY_CNT', 'DRP_EXACT_PAY_TNCY_CNT', 'DRP_UND_PAY_TNCY_CNT', 'DRP_CMLT_ARR_AMT', 'DRP_UND_PAY_CMLT_ARR_AMT', 'DRP_EXACT_PAY_ARR_TNCY_CNT', 'DRP_UND_PAY_ARR_TNCY_CNT', 'DRP_TNCY_CMLT_ADV_AMT', 'DRP_ADV_TNCY_CNT', 'DRP_HALF_RENT_TNCY_CNT', 'DRP_THRD_QTR_RENT_TNCY_CNT', 'DRP_NRML_RENT_TNCY_CNT', 'DRP_EXTRA_RENT_TNCY_CNT', 'DRP_DBL_RENT_TNCY_CNT', 'DRP_MKT_RENT_TNCY_CNT', 'DRP_CASE_CNT', 'DRP_REJ_TXN_PYMT_ITEM_AMT', 'DRP_REJ_TXN_CNT']
        df_write = df_write.select(*[col for col in _target_cols if col in df_write.columns])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "DDS_FACT_MTH_DRP_SWD_CLCT_SMRY", mode="append")

        logger.info("write_DDS_FACT_MTH_DRP_SWD_CLCT_SMRY write completed")
        logger.info("Step: write_UTL_DEV_NULL")
        # Write to Target: write_UTL_DEV_NULL
        # /dev/null — skip entire write component (no-op target in Informatica)
        logger.info("Target write_UTL_DEV_NULL is /dev/null, skipping write")

        logger.info("write_UTL_DEV_NULL write completed")
        logger.info("Step: apply_UPD_RLS_CNTL")
        # Update Strategy: apply_UPD_RLS_CNTL
        # Strategy: UPDATE_FLAG
        df_upd_11 = df_mplt_10.withColumn("_update_strategy", lit("INSERT"))
        ctx.register_df("df_upd_11", df_upd_11)
        
        logger.info("Step: write_DDS_RLS_CNTL")
        # Write to Target: write_DDS_RLS_CNTL
        df_write = df_upd_11
        # Cast columns to match target schema data types
        if "rls_cntl_fact_tbl_name" in [c.lower() for c in df_write.columns]:
            for c in df_write.columns:
                if c.lower() == "rls_cntl_fact_tbl_name":
                    df_write = df_write.withColumn(c,
                        when(col(c).cast(DecimalType(38,0)).isNotNull(),
                             col(c).cast(DecimalType(38,0)).cast(StringType()))
                        .otherwise(col(c).cast(StringType())))
        if "rls_cntl_dmns_type_code" in [c.lower() for c in df_write.columns]:
            for c in df_write.columns:
                if c.lower() == "rls_cntl_dmns_type_code":
                    df_write = df_write.withColumn(c,
                        when(col(c).cast(DecimalType(38,0)).isNotNull(),
                             col(c).cast(DecimalType(38,0)).cast(StringType()))
                        .otherwise(col(c).cast(StringType())))
        # Map source columns to target columns using connector field map (handles name mismatches)
        _field_map = {"RLS_CNTL_BGN_TIME_DMNS_KEY": "RLS_CNTL_BGN_TIME_DMNS_KEY", "RLS_CNTL_DMNS_TYPE_CODE": "RLS_CNTL_DMNS_TYPE_CODE", "RLS_CNTL_END_TIME_DMNS_KEY": "RLS_CNTL_END_TIME_DMNS_KEY", "RLS_CNTL_FACT_TBL_NAME": "RLS_CNTL_FACT_TBL_NAME"}
        for _tgt_col, _src_col in _field_map.items():
            if _tgt_col not in df_write.columns and _src_col in df_write.columns:
                df_write = df_write.withColumnRenamed(_src_col, _tgt_col)
        # Select only target-defined columns (field_map already handled name alignment)
        _target_cols = ['RLS_CNTL_FACT_TBL_NAME', 'RLS_CNTL_DMNS_TYPE_CODE', 'RLS_CNTL_BGN_TIME_DMNS_KEY', 'RLS_CNTL_END_TIME_DMNS_KEY']
        df_write = df_write.select(*[col for col in _target_cols if col in df_write.columns])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "DDS_RLS_CNTL", mode="append")

        logger.info("write_DDS_RLS_CNTL write completed")
        
        metrics.complete()
        logger.info("Mapping M_EMS_DDS_APLY_FACT_MTH_DRP_SWD_CLCT_SMRY completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_EMS_DDS_APLY_FACT_MTH_DRP_SWD_CLCT_SMRY FAILED: {e}", exc_info=True)
        return False


def main():
    """Standalone execution entry point."""
    lib.init_logger()
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_EMS_DDS_APLY_FACT_MTH_DRP_SWD_CLCT_SMRY", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_EMS_DDS_APLY_FACT_MTH_DRP_SWD_CLCT_SMRY")
    
    try:
        success = run_mapping(ctx, metrics)
        if success:
            lib._flush_pending_passwords()
        return 0 if success else 1
    finally:
        spark.stop()


if __name__ == "__main__":
    main()