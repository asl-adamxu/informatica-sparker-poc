'''
* Script - M_EMS_DDS_APLY_FACT_REHSE_ANLS.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None) -> bool:
    """
    Execute the M_EMS_DDS_APLY_FACT_REHSE_ANLS mapping transformations.
    ...
    logger.info("Start to run mapping M_EMS_DDS_APLY_FACT_REHSE_ANLS")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_EMS_DDS_APLY_FACT_REHSE_ANLS", config)
    ...
    try:
        logger.info("Step: read_DPA_FACT_EMS_REHSE_ANLS")
        # Reading Data From Source - read_DPA_FACT_EMS_REHSE_ANLS
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_DPA_FACT_EMS_REHSE_ANLS = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_REHSE_ANLS")
        
        logger.info("Step: apply_SQTRANS")
        # Source Qualifier: apply_SQTRANS
        df_SQTRANS = df_DPA_FACT_EMS_REHSE_ANLS
        # Select only SQ output ports (matches Informatica behavior)
        df_SQTRANS = df_SQTRANS.select("TIME_DMNS_KEY")
        ctx.register_df("df_SQTRANS", df_SQTRANS)
        
        logger.info("Step: apply_SQ_DPA_FACT_EMS_REHSE_ANLS")
        # Source Qualifier: apply_SQ_DPA_FACT_EMS_REHSE_ANLS
        df_SQ_DPA_FACT_EMS_REHSE_ANLS = df_DPA_FACT_EMS_REHSE_ANLS
        # Select only SQ output ports (matches Informatica behavior)
        df_SQ_DPA_FACT_EMS_REHSE_ANLS = df_SQ_DPA_FACT_EMS_REHSE_ANLS.select("EST_SCD_KEY", "DSTR_BRD_DSTR_DMNS_KEY", "DSTR_CHC_DSTR_SCD_KEY", "TFR_TYPE_DMNS_KEY", "TFR_CASE_RSN_DMNS_KEY", "TIME_DMNS_KEY", "OR_TYPE_CODE", "REHSE_NEW_CASE_CNT", "REHSE_OSTD_CASE_CNT", "REHSE_STL_CASE_CNT", "REHSE_CNCL_CASE_CNT", "REHSE_BF_CASE_CNT")
        ctx.register_df("df_SQ_DPA_FACT_EMS_REHSE_ANLS", df_SQ_DPA_FACT_EMS_REHSE_ANLS)
        
        logger.info("Step: apply_EXP_SET_DEL_INFO")
        # Expression: apply_EXP_SET_DEL_INFO
        df_EXP_SET_DEL_INFO = df_SQTRANS
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_REHSE_ANLS'"))
        ...
        _agg_input = df_SQ_DPA_FACT_EMS_REHSE_ANLS.select(
            col("REHSE_STL_CASE_CNT"),
            col("REHSE_CNCL_CASE_CNT"),
            col("REHSE_BF_CASE_CNT"),
            col("EST_SCD_KEY"),
            ...
            col("TIME_DMNS_KEY"),
            col("OR_TYPE_CODE"),
            col("REHSE_NEW_CASE_CNT"),
            col("REHSE_OSTD_CASE_CNT")        )
        ...
        df_AGGTRANS = df_AGGTRANS.agg(
            sum("REHSE_NEW_CASE_CNT").alias("REHSE_NEW_CASE_CNT1"),
            sum("REHSE_OSTD_CASE_CNT").alias("REHSE_OSTD_CASE_CNT1"),
            sum("REHSE_STL_CASE_CNT").alias("REHSE_STL_CASE_CNT1"),
            sum("REHSE_CNCL_CASE_CNT").alias("REHSE_CNCL_CASE_CNT1"),
            sum("REHSE_BF_CASE_CNT").alias("REHSE_BF_CASE_CNT1")
        )
        ...
        logger.info("Step: write_DDS_FACT_EMS_REHSE_ANLS")
        # Write to Target: write_DDS_FACT_EMS_REHSE_ANLS
        ...
        _field_map = {"DSTR_BRD_DSTR_DMNS_KEY": "DSTR_BRD_DSTR_DMNS_KEY", "DSTR_CHC_DSTR_SCD_KEY": "DSTR_CHC_DSTR_SCD_KEY", "EST_SCD_KEY": "EST_SCD_KEY", "OR_TYPE_CODE": "OR_TYPE_CODE", "REHSE_BF_CASE_CNT": "REHSE_BF_CASE_CNT1", "REHSE_CNCL_CASE_CNT": "REHSE_CNCL_CASE_CNT1", "REHSE_NEW_CASE_CNT": "REHSE_NEW_CASE_CNT1", "REHSE_OSTD_CASE_CNT": "REHSE_OSTD_CASE_CNT1", "REHSE_STL_CASE_CNT": "REHSE_STL_CASE_CNT1", "TFR_CASE_RSN_DMNS_KEY": "TFR_CASE_RSN_DMNS_KEY", "TFR_TYPE_DMNS_KEY": "TFR_TYPE_DMNS_KEY", "TIME_DMNS_KEY": "TIME_DMNS_KEY"}
        ...
        lib.write_table(df_write, conn_target, "DDS_FACT_EMS_REHSE_ANLS", mode="append")

        logger.info("write_DDS_FACT_EMS_REHSE_ANLS write completed")
        ...
        metrics.complete()
        logger.info("Mapping M_EMS_DDS_APLY_FACT_REHSE_ANLS completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_EMS_DDS_APLY_FACT_REHSE_ANLS FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_EMS_DDS_APLY_FACT_REHSE_ANLS", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_EMS_DDS_APLY_FACT_REHSE_ANLS")
