'''
* Script - M_EMS_DDS_APLY_FACT_IH_CASE_MTH_ANLS.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None) -> bool:
    """
    Execute the M_EMS_DDS_APLY_FACT_IH_CASE_MTH_ANLS mapping transformations.
    ...
    logger.info("Start to run mapping M_EMS_DDS_APLY_FACT_IH_CASE_MTH_ANLS")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_EMS_DDS_APLY_FACT_IH_CASE_MTH_ANLS", config)
    ...
    try:
        logger.info("Step: read_DPA_FACT_EMS_IH_CASE_MTH_ANLS")
        # Reading Data From Source - read_DPA_FACT_EMS_IH_CASE_MTH_ANLS
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_DPA_FACT_EMS_IH_CASE_MTH_ANLS = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_IH_CASE_MTH_ANLS")
        
        logger.info("Step: apply_SQ_DPA_FACT_EMS_IH_CASE_MTH_ANLS")
        # Source Qualifier: apply_SQ_DPA_FACT_EMS_IH_CASE_MTH_ANLS
        df_SQ_DPA_FACT_EMS_IH_CASE_MTH_ANLS = df_DPA_FACT_EMS_IH_CASE_MTH_ANLS
        # Select only SQ output ports (matches Informatica behavior)
        df_SQ_DPA_FACT_EMS_IH_CASE_MTH_ANLS = df_SQ_DPA_FACT_EMS_IH_CASE_MTH_ANLS.select("TIME_DMNS_KEY", "EST_SCD_KEY", "DSTR_BRD_DSTR_DMNS_KEY", "DSTR_CHC_DSTR_SCD_KEY", "CATG_USER_TYPE_CODE", "FLAT_TYPE_DMNS_KEY", "MAX_UNIT_HEAD_CNT", "MIN_UNIT_HEAD_CNT", "RENT_FCTR_DMNS_KEY", "OPR_TYPE_DMNS_KEY", "IH_NEW_CASE_CNT", "ACTV_TNCY_CNT")
        ctx.register_df("df_SQ_DPA_FACT_EMS_IH_CASE_MTH_ANLS", df_SQ_DPA_FACT_EMS_IH_CASE_MTH_ANLS)
        
        logger.info("Step: apply_SQTRANS")
        # Source Qualifier: apply_SQTRANS
        df_SQTRANS = df_DPA_FACT_EMS_IH_CASE_MTH_ANLS
        # Select only SQ output ports (matches Informatica behavior)
        df_SQTRANS = df_SQTRANS.select("TIME_DMNS_KEY")
        ctx.register_df("df_SQTRANS", df_SQTRANS)
        
        logger.info("Step: write_DDS_FACT_EMS_IH_CASE_MTH_ANLS")
        # Write to Target: write_DDS_FACT_EMS_IH_CASE_MTH_ANLS
        df_write = df_SQ_DPA_FACT_EMS_IH_CASE_MTH_ANLS
        ...
        lib.write_table(df_write, conn_target, "DDS_FACT_EMS_IH_CASE_MTH_ANLS", mode="append")

        logger.info("write_DDS_FACT_EMS_IH_CASE_MTH_ANLS write completed")
        logger.info("Step: apply_EXP_SET_DEL_INFO")
        # Expression: apply_EXP_SET_DEL_INFO
        df_EXP_SET_DEL_INFO = df_SQTRANS
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_IH_CASE_MTH_ANLS'"))
        ...
        metrics.complete()
        logger.info("Mapping M_EMS_DDS_APLY_FACT_IH_CASE_MTH_ANLS completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_EMS_DDS_APLY_FACT_IH_CASE_MTH_ANLS FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_EMS_DDS_APLY_FACT_IH_CASE_MTH_ANLS", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_EMS_DDS_APLY_FACT_IH_CASE_MTH_ANLS")
