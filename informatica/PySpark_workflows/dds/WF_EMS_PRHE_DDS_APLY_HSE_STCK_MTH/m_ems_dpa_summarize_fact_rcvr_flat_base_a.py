...
SELECT     	RM.UNIT_KEY as UNIT_KEY,
	...
	AND RM.TNCY_AGRMT_KEY = TAMS.TNCY_AGRMT_KEY
        ...
        df_mplt_expr_5 = df_mplt_expr_5.withColumn("END_DATE", expr("WORK_DRFT_ORD_ISS_DATE"))
        df_mplt_expr_5 = df_mplt_expr_5.withColumn("DIFF_DAY_NUM", expr("floor(datediff(V_WORK_DRFT_ORD_ISS_DATE, V_TNCY_AGRMT_TM_TRMT_DATE))"))
...
WHERE add_months(TO_DATE('$$v_rpt_mth'||'01', 'YYYYMMDD'),1)-1 between DDS_HRCHY_EMS_EST.BGN_DATE AND DDS_HRCHY_EMS_EST.END_DATE"""
        ...
        _cc = list(dict.fromkeys(c for c in df_lkp_merge_6.columns if c in df_lkp_merge_10.columns))
        if _cc:
            __lkp_dup = [c for c in df_lkp_merge_10.columns if c in df_lkp_merge_6.columns and c not in _cc]
            df_merge_11 = df_lkp_merge_6.join(
                df_lkp_merge_10.drop(*__lkp_dup) if __lkp_dup else df_lkp_merge_10,
                on=_cc, how="left"
            )
        else:
            logger.warning("No common columns between df_lkp_merge_6 and df_lkp_merge_10 — using synthetic key join")
            df_merge_11 = df_lkp_merge_6.withColumn("_join_key", lit(1)).join(
