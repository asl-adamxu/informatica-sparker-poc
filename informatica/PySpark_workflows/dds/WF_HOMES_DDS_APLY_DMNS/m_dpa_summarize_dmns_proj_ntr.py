...
select 'OTHR' proj_ntr_code, 'Others' proj_ntr_desp, 10000
  ...
  from dual"""
        ...
        __lkp_keep = [c for c in _lkp_right.columns if c.startswith("_lkp_") or c not in df_SQ_SOR_HOM_REF_PROJ_NTR.columns]
        if len(__lkp_keep) < len(_lkp_right.columns):
            _lkp_right = _lkp_right.select(*__lkp_keep)
        df_lkp_merge_1 = df_SQ_SOR_HOM_REF_PROJ_NTR.join(
            broadcast(_lkp_right),
            (df_SQ_SOR_HOM_REF_PROJ_NTR["PROJ_NTR_CODE"] == _lkp_right["_lkp_PROJ_NTR_CODE"]),
            "left"
        ).drop("_lkp_PROJ_NTR_CODE")
