...
select 'OTHR' proj_ntr_code, 'Others' proj_ntr_desp, 10000
  ...
  from dual"""
        ...
        df_SQ_SOR_HOM_REF_PROJ_NTR = df_SQ_SOR_HOM_REF_PROJ_NTR.select([col(c) if c.lower() in [x.lower() for x in df_SQ_SOR_HOM_REF_PROJ_NTR.columns] else lit(None).alias(c) for c in _port_cols])
        ...
        for _col in ["IN_PROJ_NTR_CODE", "DMNS_PROJ_NTR_KEY", "PROJ_NTR_DESP", "PROJ_NTR_CODE", "PROJ_NTR_DISP_SEQ_NUM"]:
        ...
        df_FIL_CHANGE = __fil_input.filter(expr("CHANGE_FLAG = 1 AND ( NOT (DMNS_PROJ_NTR_KEY IS NULL))"))
        ctx.register_df("df_FIL_CHANGE", df_FIL_CHANGE)

        logger.info("Step: apply_FIL_NEW")
        # Filter: apply_FIL_NEW
        __fil_input = df_EXPTRANS
        df_FIL_NEW = __fil_input.filter(expr("CHANGE_FLAG = 1 AND (DMNS_PROJ_NTR_KEY IS NULL)"))
        ...
        df_Union_Transformation_change = df_FIL_CHANGE.select(
            col("DMNS_PROJ_NTR_KEY").alias("DMNS_PROJ_NTR_KEY"),
            col("IN_PROJ_NTR_CODE").alias("IN_PROJ_NTR_CODE"),
            col("IN_PROJ_NTR_DESP").alias("IN_PROJ_NTR_DESP"),
            col("IN_DISP_SEQ_NUM").alias("IN_DISP_SEQ_NUM")        )
        df_Union_Transformation_new = df_FIL_NEW.select(
            col("NEXTVAL").alias("DMNS_PROJ_NTR_KEY"),
            col("IN_PROJ_NTR_CODE").alias("IN_PROJ_NTR_CODE"),
            col("IN_PROJ_NTR_DESP").alias("IN_PROJ_NTR_DESP"),
            col("IN_DISP_SEQ_NUM").alias("IN_DISP_SEQ_NUM")        )
        ...
        for _col in ["DMNS_PROJ_NTR_KEY", "IN_PROJ_NTR_CODE", "IN_PROJ_NTR_DESP", "IN_DISP_SEQ_NUM"]:
        ...
        df_Union_Transformation = df_Union_Transformation.select("DMNS_PROJ_NTR_KEY", "IN_PROJ_NTR_CODE", "IN_PROJ_NTR_DESP", "IN_DISP_SEQ_NUM")
