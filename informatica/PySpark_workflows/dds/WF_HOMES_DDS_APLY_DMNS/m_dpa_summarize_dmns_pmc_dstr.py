...
select 'OTHR' dstr_code, 'Others' dstr_name, 'OTHR' pms_dstr_code,
  ...
  from dual"""
        ...
        __lkp_keep = [c for c in _lkp_right.columns if c.startswith("_lkp_") or c not in df_SQ_SOR_HOM_REF_PMS_DSTR.columns]
        if len(__lkp_keep) < len(_lkp_right.columns):
            _lkp_right = _lkp_right.select(*__lkp_keep)
        df_lkp_merge_1 = df_SQ_SOR_HOM_REF_PMS_DSTR.join(
            broadcast(_lkp_right),
            (df_SQ_SOR_HOM_REF_PMS_DSTR["DSTR_CODE"] == _lkp_right["_lkp_DSTR_CODE"]) &
            (df_SQ_SOR_HOM_REF_PMS_DSTR["PMS_DSTR_CODE"] == _lkp_right["_lkp_PMS_DSTR_CODE"]),
            "left"
        ).drop("_lkp_DSTR_CODE").drop("_lkp_PMS_DSTR_CODE")
