...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None,
    ...
    try:
        ...
        df_DPA_FACT_EMS_SMS_CRT_HGST_SALE = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_SMS_CRT_HGST_SALE")
        
        logger.info("Step: apply_SQ_SP_DELETE")
        # Source Qualifier: apply_SQ_SP_DELETE
        df_SQ_SP_DELETE = df_DPA_FACT_EMS_SMS_CRT_HGST_SALE
...
From DPA_FACT_EMS_SMS_CRT_HGST_SALE"""
        ...
        df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE = lib.read_sql(spark, _conn, query=query)
        # Rename SQL result columns to SQ output ports 
        # name match first, then positional fallback (handles unaliased expressions)
        _sql_cols = df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE.columns
        ...
        for _sc in _sql_cols:
            ...
            if _pi < len(_port_cols):
                ...
                _used_ports.add(_pi)
                _pi += 1
        df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE = df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE.select(*[col(f"`{old}`").alias(new) for old, new in _rename_map.items()])
        # Select only SQ output ports (matches Informatica behavior)
        # ports the SQL didn't return become lit(None) so downstream references never fail
        df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE = df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE.select([col(c) if c.lower() in [x.lower() for x in df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE.columns] else lit(None).alias(c) for c in _port_cols])
        
        ctx.register_df("df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE", df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE)
        
        logger.info("Step: apply_EXP_SET_DEL_INFO")
        # Expression: apply_EXP_SET_DEL_INFO
        df_EXP_SET_DEL_INFO = df_SQ_SP_DELETE
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_SMS_CRT_HGST_SALE'"))
        ...
        _agg_input = df_SQ_DPA_FACT_EMS_SMS_CRT_HGST_SALE.select(
        ...
        df_AGGTRANS = _agg_input.groupBy("DESP_DTL_DMNS_KEY", "UNIT_TYPE_DMNS_KEY", "TIME_DMNS_KEY", "CRT_DMNS_KEY", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE")
