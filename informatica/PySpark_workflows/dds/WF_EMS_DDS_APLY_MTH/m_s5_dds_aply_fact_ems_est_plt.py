'''
* Script - M_S5_DDS_APLY_FACT_EMS_EST_PLT.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None) -> bool:
    """
    Execute the M_S5_DDS_APLY_FACT_EMS_EST_PLT mapping transformations.
    ...
    logger.info("Start to run mapping M_S5_DDS_APLY_FACT_EMS_EST_PLT")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_EST_PLT", config)
    ...
    try:
        logger.info("Step: read_DPA_FACT_EMS_EST_PLT")
        # Reading Data From Source - read_DPA_FACT_EMS_EST_PLT
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_DPA_FACT_EMS_EST_PLT = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_EST_PLT")
        
        logger.info("Step: apply_SQ_SP_DELETE")
        # Source Qualifier: apply_SQ_SP_DELETE
        df_SQ_SP_DELETE = df_DPA_FACT_EMS_EST_PLT
        ...
        logger.info("Step: apply_SQ_DPA_FACT_EMS_EST_PLT")
        # Source Qualifier: apply_SQ_DPA_FACT_EMS_EST_PLT
        df_SQ_DPA_FACT_EMS_EST_PLT = df_DPA_FACT_EMS_EST_PLT
        # Select only SQ output ports (matches Informatica behavior) — missing ports become lit(None)
        _port_cols = ["PLT_CNT", "AGE_GRP_DMNS_KEY", "TIME_DMNS_KEY", "EST_PLT_SCHM_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "REC_RLS_IND", "EST_PLT_TYPE_DMNS_KEY", "EST_DMNS_KEY", "DSTR_DMNS_KEY", "BLK_SCD_KEY"]
        df_SQ_DPA_FACT_EMS_EST_PLT = df_SQ_DPA_FACT_EMS_EST_PLT.select([col(c) if c in df_SQ_DPA_FACT_EMS_EST_PLT.columns else lit(None).alias(c) for c in _port_cols])
        ctx.register_df("df_SQ_DPA_FACT_EMS_EST_PLT", df_SQ_DPA_FACT_EMS_EST_PLT)
        
        logger.info("Step: apply_EXP_SET_DEL_INFO")
        # Expression: apply_EXP_SET_DEL_INFO
        df_EXP_SET_DEL_INFO = df_SQ_SP_DELETE
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_EST_PLT'"))
        ...
        _agg_input = df_SQ_DPA_FACT_EMS_EST_PLT.select(
            col("PLT_CNT"),
            col("AGE_GRP_DMNS_KEY"),
            col("TIME_DMNS_KEY"),
            col("EST_PLT_SCHM_CODE"),
            col("LAST_REC_TXN_DATE"),
            col("LAST_REC_TXN_TYPE_CODE"),
            col("EST_PLT_TYPE_DMNS_KEY"),
            col("EST_DMNS_KEY"),
            col("DSTR_DMNS_KEY"),
            col("BLK_SCD_KEY")        )
        df_AGGTRANS1 = _agg_input.groupBy("AGE_GRP_DMNS_KEY", "TIME_DMNS_KEY", "EST_PLT_SCHM_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "EST_PLT_TYPE_DMNS_KEY", "EST_DMNS_KEY", "DSTR_DMNS_KEY", "BLK_SCD_KEY")
        df_AGGTRANS1 = df_AGGTRANS1.agg(
            sum("PLT_CNT").alias("PLT_CNT_OUT"),
        ...
        logger.info("Step: write_DDS_FACT_EMS_EST_PLT")
        # Write to Target: write_DDS_FACT_EMS_EST_PLT
        ...
        _field_map = {"AGE_GRP_DMNS_KEY": "AGE_GRP_DMNS_KEY", "BLK_SCD_KEY": "BLK_SCD_KEY", "DSTR_DMNS_KEY": "DSTR_DMNS_KEY", "EST_DMNS_KEY": "EST_DMNS_KEY", "EST_PLT_SCHM_CODE": "EST_PLT_SCHM_CODE", "EST_PLT_TYPE_DMNS_KEY": "EST_PLT_TYPE_DMNS_KEY", "LAST_REC_TXN_DATE": "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "LAST_REC_TXN_TYPE_CODE", "PLT_CNT": "PLT_CNT_OUT", "REC_RLS_IND": "REC_RLS_IND", "TIME_DMNS_KEY": "TIME_DMNS_KEY"}
        ...
        lib.write_table(df_write, conn_target, "DDS_FACT_EMS_EST_PLT", mode="append")

        logger.info("write_DDS_FACT_EMS_EST_PLT write completed")
        ...
        metrics.complete()
        logger.info("Mapping M_S5_DDS_APLY_FACT_EMS_EST_PLT completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_S5_DDS_APLY_FACT_EMS_EST_PLT FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_EST_PLT", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_S5_DDS_APLY_FACT_EMS_EST_PLT")
