'''
* Script - M_S5_DDS_APLY_FACT_EMS_DSTR_COUNCIL_EST.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None) -> bool:
    """
    Execute the M_S5_DDS_APLY_FACT_EMS_DSTR_COUNCIL_EST mapping transformations.
    ...
    logger.info("Start to run mapping M_S5_DDS_APLY_FACT_EMS_DSTR_COUNCIL_EST")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_DSTR_COUNCIL_EST", config)
    ...
    try:
        logger.info("Step: read_DPA_FACT_EMS_FLAT")
        # Reading Data From Source - read_DPA_FACT_EMS_FLAT
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_DPA_FACT_EMS_FLAT = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_FLAT")
        
        logger.info("Step: apply_SQ_SP_DELETE")
        # Source Qualifier: apply_SQ_SP_DELETE
        df_SQ_SP_DELETE = df_DPA_FACT_EMS_FLAT
        ...
        logger.info("Step: apply_SQ_DPA_FACT_EMS_FLAT")
        # Source Qualifier: apply_SQ_DPA_FACT_EMS_FLAT
        df_SQ_DPA_FACT_EMS_FLAT = df_DPA_FACT_EMS_FLAT
        # Select only SQ output ports (matches Informatica behavior) — missing ports become lit(None)
        _port_cols = ["BLK_CNT", "GEOG_DSTR_NAME", "DSTR_CNL_NAME", "TNCY_SCHM_TYPE_DMNS_KEY", "TIME_DMNS_KEY", "RGN_DMNS_KEY", "EST_DMNS_KEY", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "REC_RLS_IND", "MGT_DSTR_CODE", "BLK_SCD_KEY"]
        df_SQ_DPA_FACT_EMS_FLAT = df_SQ_DPA_FACT_EMS_FLAT.select([col(c) if c in df_SQ_DPA_FACT_EMS_FLAT.columns else lit(None).alias(c) for c in _port_cols])
        ctx.register_df("df_SQ_DPA_FACT_EMS_FLAT", df_SQ_DPA_FACT_EMS_FLAT)
        
        logger.info("Step: apply_EXP_SET_DEL_INFO")
        # Expression: apply_EXP_SET_DEL_INFO
        df_EXP_SET_DEL_INFO = df_SQ_SP_DELETE
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_FLAT'"))
        ...
        _agg_input = df_SQ_DPA_FACT_EMS_FLAT.select(
            col("BLK_CNT"),
            col("GEOG_DSTR_NAME"),
            col("DSTR_CNL_NAME"),
            col("TNCY_SCHM_TYPE_DMNS_KEY"),
            col("TIME_DMNS_KEY"),
            col("RGN_DMNS_KEY"),
            col("EST_DMNS_KEY"),
            ...
            col("MGT_DSTR_CODE"),
            col("BLK_SCD_KEY")        )
        df_AGGTRANS = _agg_input.groupBy("GEOG_DSTR_NAME", "DSTR_CNL_NAME", "TNCY_SCHM_TYPE_DMNS_KEY", "TIME_DMNS_KEY", "RGN_DMNS_KEY", "EST_DMNS_KEY", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "MGT_DSTR_CODE", "BLK_SCD_KEY")
        df_AGGTRANS = df_AGGTRANS.agg(
            sum("BLK_CNT").alias("BLK_CNT_OUT"),
        ...
        logger.info("Step: write_DDS_FACT_EMS_FLAT")
        # Write to Target: write_DDS_FACT_EMS_FLAT
        ...
        _field_map = {"BLK_CNT": "BLK_CNT_OUT", "BLK_SCD_KEY": "BLK_SCD_KEY", "DSTR_CNL_NAME": "DSTR_CNL_NAME", "EST_DMNS_KEY": "EST_DMNS_KEY", "GEOG_DSTR_NAME": "GEOG_DSTR_NAME", "LAST_REC_TXN_DATE": "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "LAST_REC_TXN_TYPE_CODE", "MGT_DSTR_CODE": "MGT_DSTR_CODE", "REC_RLS_IND": "REC_RLS_IND", "RGN_DMNS_KEY": "RGN_DMNS_KEY", "TIME_DMNS_KEY": "TIME_DMNS_KEY", "TNCY_SCHM_TYPE_DMNS_KEY": "TNCY_SCHM_TYPE_DMNS_KEY"}
        ...
        logger.info("write_DDS_FACT_EMS_FLAT write completed")
        ...
        metrics.complete()
        logger.info("Mapping M_S5_DDS_APLY_FACT_EMS_DSTR_COUNCIL_EST completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_S5_DDS_APLY_FACT_EMS_DSTR_COUNCIL_EST FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_DSTR_COUNCIL_EST", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_S5_DDS_APLY_FACT_EMS_DSTR_COUNCIL_EST")
