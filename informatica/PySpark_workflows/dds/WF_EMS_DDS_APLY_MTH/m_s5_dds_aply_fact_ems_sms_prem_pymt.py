'''
* Script - M_S5_DDS_APLY_FACT_EMS_SMS_PREM_PYMT.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None,
    ...
    """
    Execute the M_S5_DDS_APLY_FACT_EMS_SMS_PREM_PYMT mapping transformations.
    ...
    logger.info("Start to run mapping M_S5_DDS_APLY_FACT_EMS_SMS_PREM_PYMT")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_SMS_PREM_PYMT", config)
    ...
    try:
        logger.info("Step: read_DPA_FACT_EMS_SMS_PREM_PYMT")
        # Reading Data From Source - read_DPA_FACT_EMS_SMS_PREM_PYMT
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_DPA_FACT_EMS_SMS_PREM_PYMT = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_SMS_PREM_PYMT")
        
        logger.info("Step: apply_SQ_SP_DELETE")
        # Source Qualifier: apply_SQ_SP_DELETE
        df_SQ_SP_DELETE = df_DPA_FACT_EMS_SMS_PREM_PYMT
        ...
        logger.info("Step: apply_SQ_DPA_FACT_EMS_SMS_PREM_PYMT")
        # Source Qualifier: apply_SQ_DPA_FACT_EMS_SMS_PREM_PYMT
        df_SQ_DPA_FACT_EMS_SMS_PREM_PYMT = df_DPA_FACT_EMS_SMS_PREM_PYMT
        # Select only SQ output ports (matches Informatica behavior) — missing ports become lit(None)
        _port_cols = ["UNIT_TYPE_DMNS_KEY", "DESP_DTL_DMNS_KEY", "TIME_DMNS_KEY", "PREM_PYMT_AMT", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "REC_RLS_IND"]
        df_SQ_DPA_FACT_EMS_SMS_PREM_PYMT = df_SQ_DPA_FACT_EMS_SMS_PREM_PYMT.select([col(c) if c.lower() in [x.lower() for x in df_SQ_DPA_FACT_EMS_SMS_PREM_PYMT.columns] else lit(None).alias(c) for c in _port_cols])
        ctx.register_df("df_SQ_DPA_FACT_EMS_SMS_PREM_PYMT", df_SQ_DPA_FACT_EMS_SMS_PREM_PYMT)
        
        logger.info("Step: apply_EXP_SET_DEL_INFO")
        # Expression: apply_EXP_SET_DEL_INFO
        df_EXP_SET_DEL_INFO = df_SQ_SP_DELETE
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_SMS_PREM_PYMT'"))
        ...
        _agg_input = df_SQ_DPA_FACT_EMS_SMS_PREM_PYMT.select(
        ...
        df_AGGTRANS = _agg_input.groupBy("UNIT_TYPE_DMNS_KEY", "DESP_DTL_DMNS_KEY", "TIME_DMNS_KEY", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE")
        df_AGGTRANS = df_AGGTRANS.agg(
            sum("PREM_PYMT_AMT").alias("PREM_PYMT_AMT_OUT"),
        ...
        logger.info("Step: write_DDS_FACT_EMS_SMS_PREM_PYMT")
        # Write to Target: write_DDS_FACT_EMS_SMS_PREM_PYMT
        df_write = df_AGGTRANS
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"DESP_DTL_DMNS_KEY": "DESP_DTL_DMNS_KEY", "LAST_REC_TXN_DATE": "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "LAST_REC_TXN_TYPE_CODE", "PREM_PYMT_AMT": "PREM_PYMT_AMT_OUT", "REC_RLS_IND": "REC_RLS_IND", "TIME_DMNS_KEY": "TIME_DMNS_KEY", "UNIT_TYPE_DMNS_KEY": "UNIT_TYPE_DMNS_KEY"}
        ...
        lib.write_table(df_write, conn_target, "DDS_FACT_EMS_SMS_PREM_PYMT", mode="append")

        logger.info("write_DDS_FACT_EMS_SMS_PREM_PYMT write completed")
        ...
        metrics.complete()
        logger.info("Mapping M_S5_DDS_APLY_FACT_EMS_SMS_PREM_PYMT completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_S5_DDS_APLY_FACT_EMS_SMS_PREM_PYMT FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_SMS_PREM_PYMT", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_S5_DDS_APLY_FACT_EMS_SMS_PREM_PYMT")
