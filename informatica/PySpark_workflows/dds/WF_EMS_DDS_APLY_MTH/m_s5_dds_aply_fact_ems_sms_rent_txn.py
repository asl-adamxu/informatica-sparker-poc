'''
* Script - M_S5_DDS_APLY_FACT_EMS_SMS_RENT_TXN.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None,
    ...
    """
    Execute the M_S5_DDS_APLY_FACT_EMS_SMS_RENT_TXN mapping transformations.
    ...
    logger.info("Start to run mapping M_S5_DDS_APLY_FACT_EMS_SMS_RENT_TXN")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_SMS_RENT_TXN", config)
    ...
    try:
        logger.info("Step: read_DPA_FACT_EMS_SMS_RENT_TXN")
        # Reading Data From Source - read_DPA_FACT_EMS_SMS_RENT_TXN
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_DPA_FACT_EMS_SMS_RENT_TXN = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_SMS_RENT_TXN")
        
        logger.info("Step: apply_SQ_SP_DELETE")
        # Source Qualifier: apply_SQ_SP_DELETE
        df_SQ_SP_DELETE = df_DPA_FACT_EMS_SMS_RENT_TXN
        ...
        df_SQ_DPA_FACT_EMS_SMS_RENT_TXN = df_DPA_FACT_EMS_SMS_RENT_TXN
        # Select only SQ output ports (matches Informatica behavior) — missing ports become lit(None)
        _port_cols = ["RENT_IND_DESP_DMNS_KEY", "DESP_DTL_DMNS_KEY", "TIME_DMNS_KEY", "TXN_CNT", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "REC_RLS_IND"]
        ...
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_SMS_RENT_TXN'"))
        ...
        _agg_input = df_SQ_DPA_FACT_EMS_SMS_RENT_TXN.select(
            col("RENT_IND_DESP_DMNS_KEY"),
            col("DESP_DTL_DMNS_KEY"),
            col("TIME_DMNS_KEY"),
            col("TXN_CNT"),
            col("LAST_REC_TXN_DATE"),
            col("LAST_REC_TXN_TYPE_CODE")        )
        df_AGGTRANS = _agg_input.groupBy("RENT_IND_DESP_DMNS_KEY", "DESP_DTL_DMNS_KEY", "TIME_DMNS_KEY", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE")
        ...
        logger.info("Step: write_DDS_FACT_EMS_SMS_RENT_TXN")
        # Write to Target: write_DDS_FACT_EMS_SMS_RENT_TXN
        df_write = df_AGGTRANS
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"DESP_DTL_DMNS_KEY": "DESP_DTL_DMNS_KEY", "LAST_REC_TXN_DATE": "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "LAST_REC_TXN_TYPE_CODE", "REC_RLS_IND": "REC_RLS_IND", "RENT_IND_DESP_DMNS_KEY": "RENT_IND_DESP_DMNS_KEY", "TIME_DMNS_KEY": "TIME_DMNS_KEY", "TXN_CNT": "TXN_CNT_OUT"}
        ...
        lib.write_table(df_write, conn_target, "DDS_FACT_EMS_SMS_RENT_TXN", mode="append")

        logger.info("write_DDS_FACT_EMS_SMS_RENT_TXN write completed")
        ...
        metrics.complete()
        logger.info("Mapping M_S5_DDS_APLY_FACT_EMS_SMS_RENT_TXN completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_S5_DDS_APLY_FACT_EMS_SMS_RENT_TXN FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_SMS_RENT_TXN", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_S5_DDS_APLY_FACT_EMS_SMS_RENT_TXN")
