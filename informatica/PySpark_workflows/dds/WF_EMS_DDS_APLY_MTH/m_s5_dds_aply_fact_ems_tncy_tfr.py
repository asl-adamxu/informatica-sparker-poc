'''
* Script - M_S5_DDS_APLY_FACT_EMS_TNCY_TFR.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None,
    ...
    """
    Execute the M_S5_DDS_APLY_FACT_EMS_TNCY_TFR mapping transformations.
    ...
    logger.info("Start to run mapping M_S5_DDS_APLY_FACT_EMS_TNCY_TFR")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_TNCY_TFR", config)
    ...
    try:
        logger.info("Step: read_DPA_FACT_EMS_TNCY_TFR")
        # Reading Data From Source - read_DPA_FACT_EMS_TNCY_TFR
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_DPA_FACT_EMS_TNCY_TFR = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_TNCY_TFR")
        
        logger.info("Step: apply_SQ_SP_DELETE")
        # Source Qualifier: apply_SQ_SP_DELETE
        df_SQ_SP_DELETE = df_DPA_FACT_EMS_TNCY_TFR
        ...
        logger.info("Step: apply_SQ_DPA_FACT_EMS_TNCY_TFR")
        # Source Qualifier: apply_SQ_DPA_FACT_EMS_TNCY_TFR
        df_SQ_DPA_FACT_EMS_TNCY_TFR = df_DPA_FACT_EMS_TNCY_TFR
        # Select only SQ output ports (matches Informatica behavior) — missing ports become lit(None)
        _port_cols = ["TNCY_TFR_CASE_CNT", "TIME_DMNS_KEY", "TFR_CASE_RSN_DMNS_KEY", "TFR_CASE_STS_DMNS_KEY", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "REC_RLS_IND", "RGN_DMNS_KEY", "DSTR_DMNS_KEY"]
        df_SQ_DPA_FACT_EMS_TNCY_TFR = df_SQ_DPA_FACT_EMS_TNCY_TFR.select([col(c) if c.lower() in [x.lower() for x in df_SQ_DPA_FACT_EMS_TNCY_TFR.columns] else lit(None).alias(c) for c in _port_cols])
        ctx.register_df("df_SQ_DPA_FACT_EMS_TNCY_TFR", df_SQ_DPA_FACT_EMS_TNCY_TFR)
        
        logger.info("Step: apply_EXP_SET_DEL_INFO")
        # Expression: apply_EXP_SET_DEL_INFO
        df_EXP_SET_DEL_INFO = df_SQ_SP_DELETE
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_TNCY_TFR'"))
        ...
        _agg_input = df_SQ_DPA_FACT_EMS_TNCY_TFR.select(
            col("TNCY_TFR_CASE_CNT"),
            col("TIME_DMNS_KEY"),
            col("TFR_CASE_RSN_DMNS_KEY"),
            col("TFR_CASE_STS_DMNS_KEY"),
            col("LAST_REC_TXN_DATE"),
            col("LAST_REC_TXN_TYPE_CODE"),
            col("RGN_DMNS_KEY"),
            col("DSTR_DMNS_KEY")        )
        df_AGGTRANS1 = _agg_input.groupBy("TIME_DMNS_KEY", "TFR_CASE_RSN_DMNS_KEY", "TFR_CASE_STS_DMNS_KEY", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "RGN_DMNS_KEY", "DSTR_DMNS_KEY")
        df_AGGTRANS1 = df_AGGTRANS1.agg(
            sum("TNCY_TFR_CASE_CNT").alias("TNCY_TFR_CASE_CNT_OUT"),
        ...
        logger.info("Step: write_DDS_FACT_EMS_TNCY_TFR")
        # Write to Target: write_DDS_FACT_EMS_TNCY_TFR
        df_write = df_AGGTRANS1
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"DSTR_DMNS_KEY": "DSTR_DMNS_KEY", "LAST_REC_TXN_DATE": "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "LAST_REC_TXN_TYPE_CODE", "REC_RLS_IND": "REC_RLS_IND", "RGN_DMNS_KEY": "RGN_DMNS_KEY", "TFR_CASE_RSN_DMNS_KEY": "TFR_CASE_RSN_DMNS_KEY", "TFR_CASE_STS_DMNS_KEY": "TFR_CASE_STS_DMNS_KEY", "TIME_DMNS_KEY": "TIME_DMNS_KEY", "TNCY_TFR_CASE_CNT": "TNCY_TFR_CASE_CNT_OUT"}
        ...
        lib.write_table(df_write, conn_target, "DDS_FACT_EMS_TNCY_TFR", mode="append")

        logger.info("write_DDS_FACT_EMS_TNCY_TFR write completed")
        ...
        metrics.complete()
        logger.info("Mapping M_S5_DDS_APLY_FACT_EMS_TNCY_TFR completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_S5_DDS_APLY_FACT_EMS_TNCY_TFR FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_TNCY_TFR", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_S5_DDS_APLY_FACT_EMS_TNCY_TFR")
