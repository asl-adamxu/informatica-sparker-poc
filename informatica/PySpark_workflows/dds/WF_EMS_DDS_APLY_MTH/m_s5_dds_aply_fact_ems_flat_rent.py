'''
* Script - M_S5_DDS_APLY_FACT_EMS_FLAT_RENT.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None) -> bool:
    ...
    logger.info("Start to run mapping M_S5_DDS_APLY_FACT_EMS_FLAT_RENT")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_FLAT_RENT", config)
    ...
    try:
        logger.info("Step: read_DPA_FACT_EMS_FLAT_RENT")
        # Reading Data From Source - read_DPA_FACT_EMS_FLAT_RENT
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "DPA")
        df_DPA_FACT_EMS_FLAT_RENT = lib.read_sql(spark, _conn, table="DPA_FACT_EMS_FLAT")
        ...
        _agg_input = df_SQ_DPA_FACT_EMS_FLAT_RENT.select(
            col("IFA_AREA"),
            col("UNIT_ENV_CODE"),
            col("TIME_DMNS_KEY"),
            ...
            col("FLAT_CNT"),
            col("EXST_RATE_AMT"),
            col("EXST_NET_RENT_AMT"),
            col("EXST_INCLD_RENT_AMT"),
            col("EXST_RENT_BGN_DATE"),
            col("NEW_RATE_AMT"),
            col("NEW_NET_RENT_AMT"),
            col("NEW_INCLD_RENT_AMT"),
            col("NEW_RENT_BGN_DATE"),
            col("EST_DMNS_KEY"),
            col("LAST_REC_TXN_DATE"),
            col("LAST_REC_TXN_TYPE_CODE"),
            col("BLK_DMNS_KEY"),
            col("TNT_RENT_CODE_CATG_CODE")        )
        df_AGGTRANS = _agg_input.groupBy("IFA_AREA", "UNIT_ENV_CODE", "TIME_DMNS_KEY", "FLAT_TYPE_DMNS_KEY", "EST_DMNS_KEY", "LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE", "BLK_DMNS_KEY", "TNT_RENT_CODE_CATG_CODE")
        df_AGGTRANS = df_AGGTRANS.agg(
            sum("FLAT_CNT").alias("FLAT_CNT_OUT"),
            sum("EXST_RATE_AMT").alias("EXST_RATE_AMT_OUT"),
            sum("EXST_NET_RENT_AMT").alias("EXST_NET_RENT_AMT_OUT"),
            sum("EXST_INCLD_RENT_AMT").alias("EXST_INCLD_RENT_AMT_OUT"),
            max("EXST_RENT_BGN_DATE").alias("EXST_RENT_BGN_DATE1"),
            sum("NEW_RATE_AMT").alias("NEW_RATE_AMT_OUT"),
            sum("NEW_NET_RENT_AMT").alias("NEW_NET_RENT_AMT_OUT"),
            sum("NEW_INCLD_RENT_AMT").alias("NEW_INCLD_RENT_AMT_OUT"),
            max("NEW_RENT_BGN_DATE").alias("NEW_RENT_BGN_DATE1"),
        ...
        df_EXP_SET_DEL_INFO = df_EXP_SET_DEL_INFO.withColumn("TBL_NAME", expr("'DDS_FACT_EMS_FLAT_RENT'"))
        ...
        logger.info("Step: write_DDS_FACT_EMS_FLAT_RENT")
        # Write to Target: write_DDS_FACT_EMS_FLAT_RENT
        ...
        df_write = df_write.withColumn("GEOG_DSTR_NAME", lit(None).cast(StringType()))
        df_write = df_write.withColumn("DSTR_CNL_NAME", lit(None).cast(StringType()))
        df_write = df_write.withColumn("TNCY_SCHM_TYPE_DMNS_KEY", lit(None).cast(StringType()))
        df_write = df_write.withColumn("RGN_DMNS_KEY", lit(None).cast(StringType()))
        df_write = df_write.withColumn("MGT_DSTR_CODE", lit(None).cast(StringType()))
        ...
        logger.info("write_DDS_FACT_EMS_FLAT_RENT write completed")
        ...
        metrics.complete()
        logger.info("Mapping M_S5_DDS_APLY_FACT_EMS_FLAT_RENT completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_S5_DDS_APLY_FACT_EMS_FLAT_RENT FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_S5_DDS_APLY_FACT_EMS_FLAT_RENT", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_S5_DDS_APLY_FACT_EMS_FLAT_RENT")
