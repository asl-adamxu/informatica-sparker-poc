...
FROM EMS_WTP_RVW_YEAR"""
        ...
        _sql_cols = df_SQ_EMS_WTP_RVW_YEAR.columns
        _port_cols = ["WTP_CYCL_YEAR_MTH", "LAST_REC_TXN_TYPE_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_USER_ID"]
        _rename_map = {}
        _used_ports = set()
        ...
        for _sc in _sql_cols:
            ...
            if _pi < len(_port_cols):
                ...
                _used_ports.add(_pi)
                _pi += 1
        df_SQ_EMS_WTP_RVW_YEAR = df_SQ_EMS_WTP_RVW_YEAR.select(*[col(f"`{old}`").alias(new) for old, new in _rename_map.items()])
        # Select only SQ output ports (matches Informatica behavior)
        # ports the SQL didn't return become lit(None) so downstream references never fail
        df_SQ_EMS_WTP_RVW_YEAR = df_SQ_EMS_WTP_RVW_YEAR.select([col(c) if c.lower() in [x.lower() for x in df_SQ_EMS_WTP_RVW_YEAR.columns] else lit(None).alias(c) for c in _port_cols])
        
        ctx.register_df("df_SQ_EMS_WTP_RVW_YEAR", df_SQ_EMS_WTP_RVW_YEAR)
        
        logger.info("Step: apply_EXP_BK")
        # Expression: apply_EXP_BK
        df_EXP_BK = df_SQ_EMS_WTP_RVW_YEAR
        df_EXP_BK = df_EXP_BK.withColumn("DUMMY_DATE", expr("NULL"))
        # Ensure any missing pass-through columns exist (no connector feeding them)
        for _col in ["WTP_CYCL_YEAR_MTH", "LAST_REC_TXN_TYPE_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_USER_ID"]:
        ...
        _dup_cnt = df_DLKP_SOR_MSTR.groupBy(col("WTP_CYCL_YEAR_MTH")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        # Join condition: IN_WTP_CYCL_YEAR_MTH=WTP_CYCL_YEAR_MTH
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_lkp_merge_EXP_BK = _lkp_input.alias("_main").join(
            broadcast(df_DLKP_SOR_MSTR).alias("_lkp"),
            (col("_main.IN_WTP_CYCL_YEAR_MTH") == col("_lkp.WTP_CYCL_YEAR_MTH")),
            "left"
        ).select(
...
where SOR_EMS_WTP_RVW_YEAR_STS.WTP_CYCL_YEAR_MTH_KEY = ss.WTP_CYCL_YEAR_MTH_KEY and SOR_EMS_WTP_RVW_YEAR_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_EMS_WTP_RVW_YEAR_STS.WTP_CYCL_YEAR_MTH_KEY """
        ...
        # Report Error on multiple match: check for duplicate join keys
        _dup_cnt = df_DLKP_SOR_STS.groupBy(col("WTP_CYCL_YEAR_MTH_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        # Join condition: IN_WTP_CYCL_YEAR_MTH_KEY=WTP_CYCL_YEAR_MTH_KEY
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_lkp_merge_EXP_BK = _lkp_input.alias("_main").join(
            broadcast(df_DLKP_SOR_STS).alias("_lkp"),
            (col("_main.IN_WTP_CYCL_YEAR_MTH_KEY") == col("_lkp.WTP_CYCL_YEAR_MTH_KEY")),
            "left"
        ).select(
            *[_lkp_input[c] for c in _lkp_input.columns],
        ...
        _nlr_key = "WTP_CYCL_YEAR_MTH_KEY"
        if (_nlr_key.lower() not in [x.lower() for x in _lkp_input.columns]
                and _nlr_key.lower() in [x.lower() for x in _nlr_lkp_cols]):
            _nlr_lkp_cols = [c for c in _nlr_lkp_cols if c.lower() != _nlr_key.lower()]
            _nlr_lkp_cols.insert(0, _nlr_key)
        ...
        df_EXPTRANS_MSTR = df_EXPTRANS_MSTR.withColumn("v_SSAL2_TBL_NAME", expr("'SSA_EMS_WTP_RVW_YEAR'"))
        ...
        _field_map = {"AGMT_IND": "OUT_AGMT_IND", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "OPR_IND": "OUT_V_OPR_IND", "WTP_CYCL_YEAR_MTH": "WTP_CYCL_YEAR_MTH", "WTP_CYCL_YEAR_MTH_KEY": "WTP_CYCL_YEAR_MTH_KEY"}
        ...
        _del_key_cols = ['wtp_cycl_year_mth_key', 'wtp_cycl_year_mth', 'agmt_ind', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            ...
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_EMS_WTP_RVW_YEAR", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['wtp_cycl_year_mth_key', 'wtp_cycl_year_mth', 'agmt_ind', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
            ...
            if _upd_set_cols:
                ...
                lib.batch_update(spark, conn_target, "SSA_EMS_WTP_RVW_YEAR", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        ...
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"BGN_DATE": "OUT_V_BGN_DATE", "END_DATE": "OUT_V_END_DATE", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "OPR_IND": "OUT_V_OPR_IND", "SOR_DATE": "OUT_SOR_DATE", "WTP_CYCL_YEAR_MTH_KEY": "WTP_CYCL_YEAR_MTH_KEY"}
        for _tgt_col, _src_col in _field_map.items():
            if _tgt_col.lower() not in [x.lower() for x in df_write.columns] and _src_col.lower() in [x.lower() for x in df_write.columns]:
                # Drop any column that would conflict case-insensitively with the target name 
                for _c in list(df_write.columns):
        ...
        df_write = df_write.drop("_update_flag")
        # DELETE: composite key batch delete (all key columns together)
        _del_key_cols = ['wtp_cycl_year_mth_key', 'bgn_date', 'end_date', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            _del_rows = [tuple(r[c] for c in _del_key_cols) for r in _df_del.select(*_del_key_cols).distinct().collect()]
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_EMS_WTP_RVW_YEAR_STS", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['wtp_cycl_year_mth_key', 'bgn_date', 'end_date', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
            _upd_set_cols = [c for c in _df_upd.columns if c.lower() not in [k.lower() for k in _upd_key_cols]]
            if _upd_set_cols:
                _upd_rows = [tuple(r[c] for c in _upd_set_cols + _upd_key_cols) for r in _df_upd.collect()]
                lib.batch_update(spark, conn_target, "SSA_EMS_WTP_RVW_YEAR_STS", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        # INSERT — set df_write to _df_ins so it flows through normal write path
        df_write = _df_ins
        # Select only target-defined columns (field_map already handled name alignment)
        _target_cols = ['WTP_CYCL_YEAR_MTH_KEY', 'BGN_DATE', 'END_DATE', 'LAST_REC_TXN_TYPE_CODE', 'LAST_REC_TXN_DATE', 'OPR_IND', 'SOR_DATE']
        df_write = df_write.select(*[col for col in _target_cols if col.lower() in [x.lower() for x in df_write.columns]])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "SSA_EMS_WTP_RVW_YEAR_STS", mode="append")
