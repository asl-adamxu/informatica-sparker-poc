...
FROM EMS_REF_APLY_SBMT_CHNL"""
        ...
        _dup_cnt = df_DLKP_SOR_MSTR.groupBy(col("APLY_SBMT_CHNL_CODE")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        # Join condition: IN_APLY_SBMT_CHNL_CODE=APLY_SBMT_CHNL_CODE
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_lkp_merge_EXP_BK = _lkp_input.alias("_main").join(
            broadcast(df_DLKP_SOR_MSTR).alias("_lkp"),
            (col("_main.IN_APLY_SBMT_CHNL_CODE") == col("_lkp.APLY_SBMT_CHNL_CODE")),
            "left"
        ).select(
...
where SOR_EMS_REF_APLY_SBMT_CHNL_STS.APLY_SBMT_CHNL_KEY = ss.APLY_SBMT_CHNL_KEY and SOR_EMS_REF_APLY_SBMT_CHNL_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_EMS_REF_APLY_SBMT_CHNL_STS.APLY_SBMT_CHNL_KEY  """
        ...
        # Report Error on multiple match: check for duplicate join keys
        _dup_cnt = df_DLKP_SOR_STS.groupBy(col("APLY_SBMT_CHNL_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        # Join condition: IN_APLY_SBMT_CHNL_KEY=APLY_SBMT_CHNL_KEY
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_lkp_merge_EXP_BK = _lkp_input.alias("_main").join(
            broadcast(df_DLKP_SOR_STS).alias("_lkp"),
            (col("_main.IN_APLY_SBMT_CHNL_KEY") == col("_lkp.APLY_SBMT_CHNL_KEY")),
            "left"
        ).select(
            *[_lkp_input[c] for c in _lkp_input.columns],
