...
ORDER BY sts.CUST_ACCC_PYMT_KEY """
        ...
        # Report Error on multiple match: check for duplicate join keys
        _dup_cnt = df_DLKP_SOR_STS.groupBy(col("CUST_ACCC_PYMT_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        df_lkp_merge_EXPTRANS = _lkp_input.alias("_main").join(
            ...
            (col("_main.IN_CUST_ACCC_PYMT_KEY") == col("_lkp.CUST_ACCC_PYMT_KEY")),
            "left"
        ).select(
            *[_lkp_input[c] for c in _lkp_input.columns],
