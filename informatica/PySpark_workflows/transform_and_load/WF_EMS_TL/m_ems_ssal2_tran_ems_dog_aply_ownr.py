'''
* Script - M_EMS_SSAL2_TRAN_EMS_DOG_APLY_OWNR.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None,
    ...
    """
    Execute the M_EMS_SSAL2_TRAN_EMS_DOG_APLY_OWNR mapping transformations.
    ...
    logger.info("Start to run mapping M_EMS_SSAL2_TRAN_EMS_DOG_APLY_OWNR")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_EMS_SSAL2_TRAN_EMS_DOG_APLY_OWNR", config)
    ...
    try:
        logger.info("Step: apply_SQ_EMS_DOG_APLY_OWNR")
        # Source Qualifier: apply_SQ_EMS_DOG_APLY_OWNR
        # SQL Pushdown - executes Informatica SQ SQL on source database
        _conn = lib.get_db_config(config, "SSA")
        # Parameterize schema from connection config
        _schema = _conn.get("schema", "") or "SSA"
        query = f"""SELECT DOG_APLY_OWNR_KEY as DOG_APLY_OWNR_KEY, DOG_APLY_KEY as DOG_APLY_KEY, DOG_OWNR_ID_TYPE_CODE as DOG_OWNR_ID_TYPE_CODE, DOG_OWNR_ID_NUM as DOG_OWNR_ID_NUM, DOG_LIC_END_DATE as DOG_LIC_END_DATE, DOG_MCRCP_NUM as DOG_MCRCP_NUM, DOG_SPCE_TEXT as DOG_SPCE_TEXT, DOG_STS_CODE as DOG_STS_CODE, LAST_REC_TXN_TYPE_CODE as LAST_REC_TXN_TYPE_CODE, LAST_REC_TXN_DATE as LAST_REC_TXN_DATE, LAST_REC_TXN_USER_ID as LAST_REC_TXN_USER_ID 

FROM EMS_DOG_APLY_OWNR"""
        query = query.replace("$$v_snsh_date", v_snsh_date)
        query = query.replace("$$v_init_flag", v_init_flag)
        df_SQ_EMS_DOG_APLY_OWNR = lib.read_sql(spark, _conn, query=query)
        # Rename SQL result columns to SQ output ports 
        # name match first, then positional fallback (handles unaliased expressions)
        _sql_cols = df_SQ_EMS_DOG_APLY_OWNR.columns
        _port_cols = ["DOG_APLY_OWNR_KEY", "DOG_APLY_KEY", "DOG_OWNR_ID_TYPE_CODE", "DOG_OWNR_ID_NUM", "DOG_LIC_END_DATE", "DOG_MCRCP_NUM", "DOG_SPCE_TEXT", "DOG_STS_CODE", "LAST_REC_TXN_TYPE_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_USER_ID"]
        _rename_map = {}
        _used_ports = set()
        ...
        for _sc in _sql_cols:
            ...
            if _pi < len(_port_cols):
                ...
                _used_ports.add(_pi)
                _pi += 1
        df_SQ_EMS_DOG_APLY_OWNR = df_SQ_EMS_DOG_APLY_OWNR.select(*[col(f"`{old}`").alias(new) for old, new in _rename_map.items()])
        # Select only SQ output ports (matches Informatica behavior)
        # ports the SQL didn't return become lit(None) so downstream references never fail
        df_SQ_EMS_DOG_APLY_OWNR = df_SQ_EMS_DOG_APLY_OWNR.select([col(c) if c.lower() in [x.lower() for x in df_SQ_EMS_DOG_APLY_OWNR.columns] else lit(None).alias(c) for c in _port_cols])
        
        ctx.register_df("df_SQ_EMS_DOG_APLY_OWNR", df_SQ_EMS_DOG_APLY_OWNR)
        
        logger.info("Step: apply_EXP_BK")
        # Expression: apply_EXP_BK
        df_EXP_BK = df_SQ_EMS_DOG_APLY_OWNR
        df_EXP_BK = df_EXP_BK.withColumn("DUMMY_DATE", expr("NULL"))
        # Ensure any missing pass-through columns exist (no connector feeding them)
        for _col in ["DOG_APLY_OWNR_KEY", "DOG_APLY_KEY", "DOG_OWNR_ID_TYPE_CODE", "DOG_OWNR_ID_NUM", "DOG_LIC_END_DATE", "DOG_MCRCP_NUM", "DOG_SPCE_TEXT", "DOG_STS_CODE", "LAST_REC_TXN_TYPE_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_USER_ID"]:
        ...
        _conn = lib.get_db_config(config, "SSA")
        df_DLKP_SOR_MSTR = lib.read_sql(spark, _conn, table="SOR_EMS_DOG_APLY_OWNR")
        ...
        _lkp_input = _lkp_input.withColumn("IN_EMS_DOG_APLY_OWNR_KEY", col("DOG_APLY_OWNR_KEY"))
        df_lkp_merge_EXP_BK = lib.dynamic_lookup(
            spark=spark,
            input_df=_lkp_input,
            lookup_df=df_DLKP_SOR_MSTR,
            name='DLKP_SOR_MSTR',
            join_predicates=[{'source_col': 'IN_EMS_DOG_APLY_OWNR_KEY', 'lookup_col': 'EMS_DOG_APLY_OWNR_KEY'}],
            output_columns=['DOG_APLY_OWNR_KEY', 'EMS_DOG_APLY_OWNR_KEY'],
            lookup_output_fields=[
                {'name': 'DOG_APLY_OWNR_KEY', 'ref_field': 'Sequence-Id', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'integer'},
                {'name': 'EMS_DOG_APLY_OWNR_KEY', 'ref_field': 'DOG_APLY_OWNR_KEY', 'ignore_in_compare': True, 'ignore_null_inputs': False, 'datatype': 'decimal'}
            ],
            new_lookup_row_col='NewLookupRow',
            sequence_config={'output_col': 'DOG_APLY_OWNR_KEY'},
...
where SOR_EMS_DOG_APLY_OWNR_STS.DOG_APLY_OWNR_KEY = ss.DOG_APLY_OWNR_KEY and SOR_EMS_DOG_APLY_OWNR_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_EMS_DOG_APLY_OWNR_STS.DOG_APLY_OWNR_KEY  """
        ...
        _lkp_input = _lkp_input.withColumn("IN_DOG_OWNR_ID_TYPE_CODE", col("DOG_OWNR_ID_TYPE_CODE"))
        _lkp_input = _lkp_input.withColumn("IN_DOG_OWNR_ID_NUM", col("DOG_OWNR_ID_NUM"))
        _lkp_input = _lkp_input.withColumn("IN_DOG_LIC_END_DATE", col("DOG_LIC_END_DATE"))
        _lkp_input = _lkp_input.withColumn("IN_DOG_MCRCP_NUM", col("DOG_MCRCP_NUM"))
        _lkp_input = _lkp_input.withColumn("IN_DOG_SPCE_TEXT", col("DOG_SPCE_TEXT"))
        _lkp_input = _lkp_input.withColumn("IN_DOG_STS_CODE", col("DOG_STS_CODE"))
        _lkp_input = _lkp_input.withColumn("IN_DOG_APLY_OWNR_KEY", col("DOG_APLY_OWNR_KEY"))
        df_lkp_merge_EXP_BK = lib.dynamic_lookup(
            spark=spark,
            input_df=_lkp_input,
            lookup_df=df_DLKP_SOR_STS,
            name='DLKP_SOR_STS',
            join_predicates=[{'source_col': 'IN_DOG_APLY_OWNR_KEY', 'lookup_col': 'DOG_APLY_OWNR_KEY'}],
            output_columns=['DOG_APLY_OWNR_KEY', 'BGN_DATE', 'END_DATE', 'DOG_APLY_KEY', 'DOG_OWNR_ID_TYPE_CODE', 'DOG_OWNR_ID_NUM', 'DOG_LIC_END_DATE', 'DOG_MCRCP_NUM', 'DOG_SPCE_TEXT', 'DOG_STS_CODE'],
            lookup_output_fields=[
                {'name': 'DOG_APLY_OWNR_KEY', 'ref_field': 'DOG_APLY_OWNR_KEY', 'ignore_in_compare': True, 'ignore_null_inputs': False, 'datatype': 'integer'},
                ...
                {'name': 'DOG_APLY_KEY', 'ref_field': 'DOG_APLY_KEY', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'decimal'},
                {'name': 'DOG_OWNR_ID_TYPE_CODE', 'ref_field': 'DOG_OWNR_ID_TYPE_CODE', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'string'},
                {'name': 'DOG_OWNR_ID_NUM', 'ref_field': 'DOG_OWNR_ID_NUM', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'string'},
                {'name': 'DOG_LIC_END_DATE', 'ref_field': 'DOG_LIC_END_DATE', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'date/time'},
                {'name': 'DOG_MCRCP_NUM', 'ref_field': 'DOG_MCRCP_NUM', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'string'},
                {'name': 'DOG_SPCE_TEXT', 'ref_field': 'DOG_SPCE_TEXT', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'string'},
                {'name': 'DOG_STS_CODE', 'ref_field': 'DOG_STS_CODE', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'string'}
            ],
            new_lookup_row_col='NewLookupRow',
        ...
        df_EXPTRANS_MSTR = df_EXPTRANS_MSTR.withColumn("v_SSAL2_TBL_NAME", expr("'SSA_EMS_DOG_APLY_OWNR'"))
        ...
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "SSA")
        query = f"""SELECT DOG_APLY_OWNR_KEY as SURROGATE_KEY, NULL as DUMMY  FROM SSA_EMS_DOG_APLY_OWNR"""
        query = query.replace("$$v_snsh_date", v_snsh_date)
        query = query.replace("$$v_init_flag", v_init_flag)
        ...
        _lkp_input = _lkp_input.withColumn("IN_SURROGATE_KEY", col("DOG_APLY_OWNR_KEY"))
        ...
        df_lkp_merge_FILTRANS_MSTR = lib.dynamic_lookup(
            ...
            lookup_output_fields=[
                {'name': 'SURROGATE_KEY', 'ref_field': 'DOG_APLY_OWNR_KEY', 'ignore_in_compare': True, 'ignore_null_inputs': False, 'datatype': 'integer'},
        ...
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "SSA")
        query = f"""SELECT DOG_APLY_OWNR_KEY as SURROGATE_KEY, NULL as DUMMY  FROM SSA_EMS_DOG_APLY_OWNR_STS"""
        query = query.replace("$$v_snsh_date", v_snsh_date)
        query = query.replace("$$v_init_flag", v_init_flag)
        ...
        logger.info("Step: write_SSA_EMS_DOG_APLY_OWNR")
        # Write to Target: write_SSA_EMS_DOG_APLY_OWNR
        df_write = df_UPD_SSAL2_MSTR
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"AGMT_IND": "OUT_AGMT_IND", "DOG_APLY_OWNR_KEY": "DOG_APLY_OWNR_KEY", "EMS_DOG_APLY_OWNR_KEY": "EMS_DOG_APLY_OWNR_KEY", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "OPR_IND": "OUT_V_OPR_IND"}
        ...
        _del_key_cols = ['dog_aply_ownr_key', 'ems_dog_aply_ownr_key', 'agmt_ind', 'last_rec_txn_date', 'last_rec_txn_type_code', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            ...
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_EMS_DOG_APLY_OWNR", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['dog_aply_ownr_key', 'ems_dog_aply_ownr_key', 'agmt_ind', 'last_rec_txn_date', 'last_rec_txn_type_code', 'opr_ind', 'sor_date']
            ...
            if _upd_set_cols:
                ...
                lib.batch_update(spark, conn_target, "SSA_EMS_DOG_APLY_OWNR", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        ...
        _target_cols = ['DOG_APLY_OWNR_KEY', 'EMS_DOG_APLY_OWNR_KEY', 'AGMT_IND', 'LAST_REC_TXN_DATE', 'LAST_REC_TXN_TYPE_CODE', 'OPR_IND', 'SOR_DATE']
        df_write = df_write.select(*[col for col in _target_cols if col.lower() in [x.lower() for x in df_write.columns]])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "SSA_EMS_DOG_APLY_OWNR", mode="append")

        logger.info("write_SSA_EMS_DOG_APLY_OWNR write completed")
        logger.info("Step: write_SSA_EMS_DOG_APLY_OWNR_STS")
        # Write to Target: write_SSA_EMS_DOG_APLY_OWNR_STS
        df_write = df_UPD_SSAL2_STS
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"BGN_DATE": "OUT_V_BGN_DATE", "DOG_APLY_KEY": "DOG_APLY_KEY", "DOG_APLY_OWNR_KEY": "DOG_APLY_OWNR_KEY", "DOG_LIC_END_DATE": "DOG_LIC_END_DATE", "DOG_MCRCP_NUM": "DOG_MCRCP_NUM", "DOG_OWNR_ID_NUM": "DOG_OWNR_ID_NUM", "DOG_OWNR_ID_TYPE_CODE": "DOG_OWNR_ID_TYPE_CODE", "DOG_SPCE_TEXT": "DOG_SPCE_TEXT", "DOG_STS_CODE": "DOG_STS_CODE", "END_DATE": "OUT_V_END_DATE", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "OPR_IND": "OUT_V_OPR_IND", "SOR_DATE": "OUT_SOR_DATE"}
        for _tgt_col, _src_col in _field_map.items():
            if _tgt_col.lower() not in [x.lower() for x in df_write.columns] and _src_col.lower() in [x.lower() for x in df_write.columns]:
                # Drop any column that would conflict case-insensitively with the target name 
                for _c in list(df_write.columns):
        ...
        df_write = df_write.drop("_update_flag")
        # DELETE: composite key batch delete (all key columns together)
        _del_key_cols = ['dog_aply_ownr_key', 'bgn_date', 'end_date', 'dog_aply_key', 'dog_ownr_id_type_code', 'dog_ownr_id_num', 'dog_lic_end_date', 'dog_mcrcp_num', 'dog_spce_text', 'dog_sts_code', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            _del_rows = [tuple(r[c] for c in _del_key_cols) for r in _df_del.select(*_del_key_cols).distinct().collect()]
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_EMS_DOG_APLY_OWNR_STS", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['dog_aply_ownr_key', 'bgn_date', 'end_date', 'dog_aply_key', 'dog_ownr_id_type_code', 'dog_ownr_id_num', 'dog_lic_end_date', 'dog_mcrcp_num', 'dog_spce_text', 'dog_sts_code', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
            _upd_set_cols = [c for c in _df_upd.columns if c.lower() not in [k.lower() for k in _upd_key_cols]]
            if _upd_set_cols:
                _upd_rows = [tuple(r[c] for c in _upd_set_cols + _upd_key_cols) for r in _df_upd.collect()]
                lib.batch_update(spark, conn_target, "SSA_EMS_DOG_APLY_OWNR_STS", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        # INSERT — set df_write to _df_ins so it flows through normal write path
        df_write = _df_ins
        # Select only target-defined columns (field_map already handled name alignment)
        _target_cols = ['DOG_APLY_OWNR_KEY', 'BGN_DATE', 'END_DATE', 'DOG_APLY_KEY', 'DOG_OWNR_ID_TYPE_CODE', 'DOG_OWNR_ID_NUM', 'DOG_LIC_END_DATE', 'DOG_MCRCP_NUM', 'DOG_SPCE_TEXT', 'DOG_STS_CODE', 'LAST_REC_TXN_TYPE_CODE', 'LAST_REC_TXN_DATE', 'OPR_IND', 'SOR_DATE']
        df_write = df_write.select(*[col for col in _target_cols if col.lower() in [x.lower() for x in df_write.columns]])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "SSA_EMS_DOG_APLY_OWNR_STS", mode="append")

        logger.info("write_SSA_EMS_DOG_APLY_OWNR_STS write completed")
        
        metrics.complete()
        logger.info("Mapping M_EMS_SSAL2_TRAN_EMS_DOG_APLY_OWNR completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_EMS_SSAL2_TRAN_EMS_DOG_APLY_OWNR FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_EMS_SSAL2_TRAN_EMS_DOG_APLY_OWNR", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_EMS_SSAL2_TRAN_EMS_DOG_APLY_OWNR")
