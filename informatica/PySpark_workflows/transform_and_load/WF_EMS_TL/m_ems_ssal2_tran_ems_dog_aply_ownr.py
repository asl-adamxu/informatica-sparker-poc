...
FROM EMS_DOG_APLY_OWNR"""
        ...
        _dup_cnt = df_DLKP_SOR_MSTR.groupBy(col("EMS_DOG_APLY_OWNR_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        # Join condition: IN_EMS_DOG_APLY_OWNR_KEY=EMS_DOG_APLY_OWNR_KEY
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_lkp_merge_EXP_BK = _lkp_input.alias("_main").join(
            broadcast(df_DLKP_SOR_MSTR).alias("_lkp"),
            (col("_main.IN_EMS_DOG_APLY_OWNR_KEY") == col("_lkp.EMS_DOG_APLY_OWNR_KEY")),
            "left"
        ).select(
        ...
        _nlr_key = "EMS_DOG_APLY_OWNR_KEY"
...
where SOR_EMS_DOG_APLY_OWNR_STS.DOG_APLY_OWNR_KEY = ss.DOG_APLY_OWNR_KEY and SOR_EMS_DOG_APLY_OWNR_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_EMS_DOG_APLY_OWNR_STS.DOG_APLY_OWNR_KEY  """
        ...
        # Report Error on multiple match: check for duplicate join keys
        _dup_cnt = df_DLKP_SOR_STS.groupBy(col("DOG_APLY_OWNR_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        # Join condition: IN_DOG_APLY_OWNR_KEY=DOG_APLY_OWNR_KEY
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_lkp_merge_EXP_BK = _lkp_input.alias("_main").join(
            broadcast(df_DLKP_SOR_STS).alias("_lkp"),
            (col("_main.IN_DOG_APLY_OWNR_KEY") == col("_lkp.DOG_APLY_OWNR_KEY")),
            "left"
        ).select(
            *[_lkp_input[c] for c in _lkp_input.columns],
