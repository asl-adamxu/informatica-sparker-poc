...
FROM EMS_DOG_APLY_RGSTR"""
        ...
        _sql_cols = df_SQ_EMS_DOG_APLY_RGSTR.columns
        ...
        for _sc in _sql_cols:
            ...
            if _pi < len(_port_cols):
                ...
                _used_ports.add(_pi)
                _pi += 1
        df_SQ_EMS_DOG_APLY_RGSTR = df_SQ_EMS_DOG_APLY_RGSTR.select(*[col(f"`{old}`").alias(new) for old, new in _rename_map.items()])
        # Select only SQ output ports (matches Informatica behavior)
        # ports the SQL didn't return become lit(None) so downstream references never fail
        df_SQ_EMS_DOG_APLY_RGSTR = df_SQ_EMS_DOG_APLY_RGSTR.select([col(c) if c.lower() in [x.lower() for x in df_SQ_EMS_DOG_APLY_RGSTR.columns] else lit(None).alias(c) for c in _port_cols])
        
        ctx.register_df("df_SQ_EMS_DOG_APLY_RGSTR", df_SQ_EMS_DOG_APLY_RGSTR)
        
        logger.info("Step: apply_EXP_BK")
        # Expression: apply_EXP_BK
        df_EXP_BK = df_SQ_EMS_DOG_APLY_RGSTR
...
where SOR_EMS_DOG_APLY_RGSTR_STS.DOG_APLY_KEY = ss.DOG_APLY_KEY and SOR_EMS_DOG_APLY_RGSTR_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_EMS_DOG_APLY_RGSTR_STS.DOG_APLY_KEY  """
        ...
        df_EXPTRANS_MSTR = df_EXPTRANS_MSTR.withColumn("v_SSAL2_TBL_NAME", expr("'SSA_EMS_DOG_APLY_RGSTR'"))
        ...
        _field_map = {"AGMT_IND": "OUT_AGMT_IND", "DOG_APLY_KEY": "DOG_APLY_KEY", "EMS_DOG_APLY_KEY": "EMS_DOG_APLY_KEY", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "OPR_IND": "OUT_V_OPR_IND"}
        ...
        _del_key_cols = ['dog_aply_key', 'ems_dog_aply_key', 'agmt_ind', 'last_rec_txn_date', 'last_rec_txn_type_code', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            ...
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_EMS_DOG_APLY_RGSTR", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['dog_aply_key', 'ems_dog_aply_key', 'agmt_ind', 'last_rec_txn_date', 'last_rec_txn_type_code', 'opr_ind', 'sor_date']
            ...
            if _upd_set_cols:
                ...
                lib.batch_update(spark, conn_target, "SSA_EMS_DOG_APLY_RGSTR", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        ...
        _target_cols = ['DOG_APLY_KEY', 'EMS_DOG_APLY_KEY', 'AGMT_IND', 'LAST_REC_TXN_DATE', 'LAST_REC_TXN_TYPE_CODE', 'OPR_IND', 'SOR_DATE']
        df_write = df_write.select(*[col for col in _target_cols if col.lower() in [x.lower() for x in df_write.columns]])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "SSA_EMS_DOG_APLY_RGSTR", mode="append")
        ...
        df_write = _df_ins
        # Add NULL for unmapped target columns (schema parity) - excluding identity columns
        df_write = df_write.withColumn("EMS_DOG_APLY_KEY", lit(None).cast(StringType()))
