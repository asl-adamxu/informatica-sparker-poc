...
FROM EMS_CSR_APLY_RCV"""
        ...
        _dup_cnt = df_DLKP_SOR_MSTR.groupBy(col("EMS_ENQ_KEY"), col("EMS_PRVS_APLY_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        df_lkp_merge_EXP_BK = _lkp_input.alias("_main").join(
            ...
            (col("_main.IN_EMS_ENQ_KEY") == col("_lkp.EMS_ENQ_KEY")) &
            (col("_main.IN_EMS_PRVS_APLY_KEY") == col("_lkp.EMS_PRVS_APLY_KEY")),
            "left"
        ).select(
...
where SOR_EMS_CSR_APLY_RCV_STS.CSR_APLY_RCV_KEY = ss.CSR_APLY_RCV_KEY and SOR_EMS_CSR_APLY_RCV_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_EMS_CSR_APLY_RCV_STS.CSR_APLY_RCV_KEY  """
        ...
        # Report Error on multiple match: check for duplicate join keys
        _dup_cnt = df_DLKP_SOR_STS.groupBy(col("CSR_APLY_RCV_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        df_lkp_merge_EXP_BK = _lkp_input.alias("_main").join(
            ...
            (col("_main.IN_CSR_APLY_RCV_KEY") == col("_lkp.CSR_APLY_RCV_KEY")),
            "left"
        ).select(
            *[_lkp_input[c] for c in _lkp_input.columns],
