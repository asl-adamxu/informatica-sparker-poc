'''
* Script - M_EMS_SSAL2_TRAN_EMS_PHA_QPS_CHK.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None,
    ...
    """
    Execute the M_EMS_SSAL2_TRAN_EMS_PHA_QPS_CHK mapping transformations.
    ...
    logger.info("Start to run mapping M_EMS_SSAL2_TRAN_EMS_PHA_QPS_CHK")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_EMS_SSAL2_TRAN_EMS_PHA_QPS_CHK", config)
...
FROM EMS_PHA_QPS_CHK"""
        ...
        for _sc in _sql_cols:
            ...
            if _pi < len(_port_cols):
                ...
                _used_ports.add(_pi)
                _pi += 1
        df_SQ_EMS_PHA_QPS_CHK = df_SQ_EMS_PHA_QPS_CHK.select(*[col(f"`{old}`").alias(new) for old, new in _rename_map.items()])
        # Select only SQ output ports (matches Informatica behavior)
        # ports the SQL didn't return become lit(None) so downstream references never fail
        df_SQ_EMS_PHA_QPS_CHK = df_SQ_EMS_PHA_QPS_CHK.select([col(c) if c.lower() in [x.lower() for x in df_SQ_EMS_PHA_QPS_CHK.columns] else lit(None).alias(c) for c in _port_cols])
        
        ctx.register_df("df_SQ_EMS_PHA_QPS_CHK", df_SQ_EMS_PHA_QPS_CHK)
        
        logger.info("Step: apply_EXP_BK")
        # Expression: apply_EXP_BK
        df_EXP_BK = df_SQ_EMS_PHA_QPS_CHK
        ...
        _conn = lib.get_db_config(config, "SSA")
        df_DLKP_SOR_MSTR = lib.read_sql(spark, _conn, table="SOR_EMS_PHA_QPS_CHK")
        ...
        _lkp_input = _lkp_input.withColumn("IN_QPS_CHK_ID", col("QPS_CHK_ID"))
        _lkp_input = _lkp_input.withColumn("IN_QPS_CHK_STG_CODE", col("QPS_CHK_STG_CODE"))
        df_lkp_merge_EXP_BK = lib.dynamic_lookup(
            spark=spark,
            input_df=_lkp_input,
            lookup_df=df_DLKP_SOR_MSTR,
            name='DLKP_SOR_MSTR',
            join_predicates=[{'source_col': 'IN_QPS_CHK_ID', 'lookup_col': 'QPS_CHK_ID'}, {'source_col': 'IN_QPS_CHK_STG_CODE', 'lookup_col': 'QPS_CHK_STG_CODE'}],
            ...
            lookup_output_fields=[
                ...
                {'name': 'QPS_CHK_ID', 'ref_field': 'QPS_CHK_ID', 'ignore_in_compare': True, 'ignore_null_inputs': False, 'datatype': 'string'},
                {'name': 'QPS_CHK_STG_CODE', 'ref_field': 'QPS_CHK_STG_CODE', 'ignore_in_compare': True, 'ignore_null_inputs': True, 'datatype': 'string'}
            ],
...
where SOR_EMS_PHA_QPS_CHK_STS.QPS_CHK_KEY = ss.QPS_CHK_KEY and SOR_EMS_PHA_QPS_CHK_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_EMS_PHA_QPS_CHK_STS.QPS_CHK_KEY  """
        ...
        _lkp_input = _lkp_input.withColumn("IN_QPS_CHK_PNT_NUM", col("QPS_CHK_PNT_NUM"))
        _lkp_input = _lkp_input.withColumn("IN_QPS_CHK_PREV_RPLY_PCT", col("QPS_CHK_PREV_RPLY_PCT"))
        _lkp_input = _lkp_input.withColumn("IN_QPS_CHK_PREV_NO_RPLY_PCT", col("QPS_CHK_PREV_NO_RPLY_PCT"))
        _lkp_input = _lkp_input.withColumn("IN_QPS_CHK_STG_BGN_DATE", col("QPS_CHK_STG_BGN_DATE"))
        _lkp_input = _lkp_input.withColumn("IN_QPS_CHK_STG_END_DATE", col("QPS_CHK_STG_END_DATE"))
        _lkp_input = _lkp_input.withColumn("IN_QPS_CHK_PPR_VET_CNT", col("QPS_CHK_PPR_VET_CNT"))
        ...
        df_lkp_merge_EXP_BK = lib.dynamic_lookup(
            ...
            name='DLKP_SOR_STS',
            join_predicates=[{'source_col': 'IN_QPS_CHK_KEY', 'lookup_col': 'QPS_CHK_KEY'}],
            output_columns=['QPS_CHK_KEY', 'BGN_DATE', 'END_DATE', 'QPS_CHK_PNT_NUM', 'QPS_CHK_PREV_RPLY_PCT', 'QPS_CHK_PREV_NO_RPLY_PCT', 'QPS_CHK_STG_BGN_DATE', 'QPS_CHK_STG_END_DATE', 'QPS_CHK_PPR_VET_CNT', 'QPS_CHK_BTCH_REJ_DATE', 'QPS_CHK_BTCH_REJ_RPLY_DLN_DATE'],
            lookup_output_fields=[
                {'name': 'QPS_CHK_KEY', 'ref_field': 'QPS_CHK_KEY', 'ignore_in_compare': True, 'ignore_null_inputs': False, 'datatype': 'integer'},
                ...
                {'name': 'QPS_CHK_PNT_NUM', 'ref_field': 'QPS_CHK_PNT_NUM', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'decimal'},
                {'name': 'QPS_CHK_PREV_RPLY_PCT', 'ref_field': 'QPS_CHK_PREV_RPLY_PCT', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'decimal'},
                {'name': 'QPS_CHK_PREV_NO_RPLY_PCT', 'ref_field': 'QPS_CHK_PREV_NO_RPLY_PCT', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'decimal'},
                {'name': 'QPS_CHK_STG_BGN_DATE', 'ref_field': 'QPS_CHK_STG_BGN_DATE', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'date/time'},
                {'name': 'QPS_CHK_STG_END_DATE', 'ref_field': 'QPS_CHK_STG_END_DATE', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'date/time'},
                {'name': 'QPS_CHK_PPR_VET_CNT', 'ref_field': 'QPS_CHK_PPR_VET_CNT', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'decimal'},
        ...
        df_EXPTRANS_MSTR = df_EXPTRANS_MSTR.withColumn("v_SSAL2_TBL_NAME", expr("'SSA_EMS_PHA_QPS_CHK'"))
        ...
        _lkp_input = _lkp_input.withColumn("IN_SURROGATE_KEY", col("QPS_CHK_KEY"))
        ...
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "SSA")
        query = f"""SELECT QPS_CHK_KEY as SURROGATE_KEY, NULL as DUMMY  FROM SSA_EMS_PHA_QPS_CHK_STS"""
        query = query.replace("$$v_snsh_date", v_snsh_date)
        query = query.replace("$$v_init_flag", v_init_flag)
        ...
        _field_map = {"AGMT_IND": "OUT_AGMT_IND", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "OPR_IND": "OUT_V_OPR_IND", "QPS_CHK_ID": "QPS_CHK_ID", "QPS_CHK_KEY": "QPS_CHK_KEY", "QPS_CHK_STG_CODE": "QPS_CHK_STG_CODE"}
        ...
        logger.info("Step: write_SSA_EMS_PHA_QPS_CHK_STS")
        # Write to Target: write_SSA_EMS_PHA_QPS_CHK_STS
        df_write = df_UPD_SSAL2_STS
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"BGN_DATE": "OUT_V_BGN_DATE", "END_DATE": "OUT_V_END_DATE", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "OPR_IND": "OUT_V_OPR_IND", "QPS_CHK_BTCH_REJ_DATE": "QPS_CHK_BTCH_REJ_DATE", "QPS_CHK_BTCH_REJ_RPLY_DLN_DATE": "QPS_CHK_BTCH_REJ_RPLY_DLN_DATE", "QPS_CHK_KEY": "QPS_CHK_KEY", "QPS_CHK_PNT_NUM": "QPS_CHK_PNT_NUM", "QPS_CHK_PPR_VET_CNT": "QPS_CHK_PPR_VET_CNT", "QPS_CHK_PREV_NO_RPLY_PCT": "QPS_CHK_PREV_NO_RPLY_PCT", "QPS_CHK_PREV_RPLY_PCT": "QPS_CHK_PREV_RPLY_PCT", "QPS_CHK_STG_BGN_DATE": "QPS_CHK_STG_BGN_DATE", "QPS_CHK_STG_END_DATE": "QPS_CHK_STG_END_DATE", "SOR_DATE": "OUT_SOR_DATE"}
        for _tgt_col, _src_col in _field_map.items():
            if _tgt_col.lower() not in [x.lower() for x in df_write.columns] and _src_col.lower() in [x.lower() for x in df_write.columns]:
                # Drop any column that would conflict case-insensitively with the target name 
                for _c in list(df_write.columns):
        ...
        df_write = df_write.drop("_update_flag")
        # DELETE: composite key batch delete (all key columns together)
        _del_key_cols = ['qps_chk_key', 'bgn_date', 'end_date', 'qps_chk_pnt_num', 'qps_chk_prev_rply_pct', 'qps_chk_prev_no_rply_pct', 'qps_chk_stg_bgn_date', 'qps_chk_stg_end_date', 'qps_chk_ppr_vet_cnt', 'qps_chk_btch_rej_date', 'qps_chk_btch_rej_rply_dln_date', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            _del_rows = [tuple(r[c] for c in _del_key_cols) for r in _df_del.select(*_del_key_cols).distinct().collect()]
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_EMS_PHA_QPS_CHK_STS", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['qps_chk_key', 'bgn_date', 'end_date', 'qps_chk_pnt_num', 'qps_chk_prev_rply_pct', 'qps_chk_prev_no_rply_pct', 'qps_chk_stg_bgn_date', 'qps_chk_stg_end_date', 'qps_chk_ppr_vet_cnt', 'qps_chk_btch_rej_date', 'qps_chk_btch_rej_rply_dln_date', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
            _upd_set_cols = [c for c in _df_upd.columns if c.lower() not in [k.lower() for k in _upd_key_cols]]
            if _upd_set_cols:
                _upd_rows = [tuple(r[c] for c in _upd_set_cols + _upd_key_cols) for r in _df_upd.collect()]
                lib.batch_update(spark, conn_target, "SSA_EMS_PHA_QPS_CHK_STS", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        # INSERT — set df_write to _df_ins so it flows through normal write path
        df_write = _df_ins
        # Select only target-defined columns (field_map already handled name alignment)
        _target_cols = ['QPS_CHK_KEY', 'BGN_DATE', 'END_DATE', 'QPS_CHK_PNT_NUM', 'QPS_CHK_PREV_RPLY_PCT', 'QPS_CHK_PREV_NO_RPLY_PCT', 'QPS_CHK_STG_BGN_DATE', 'QPS_CHK_STG_END_DATE', 'QPS_CHK_PPR_VET_CNT', 'QPS_CHK_BTCH_REJ_DATE', 'QPS_CHK_BTCH_REJ_RPLY_DLN_DATE', 'LAST_REC_TXN_TYPE_CODE', 'LAST_REC_TXN_DATE', 'OPR_IND', 'SOR_DATE']
        df_write = df_write.select(*[col for col in _target_cols if col.lower() in [x.lower() for x in df_write.columns]])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "SSA_EMS_PHA_QPS_CHK_STS", mode="append")

        logger.info("write_SSA_EMS_PHA_QPS_CHK_STS write completed")
        
        metrics.complete()
        logger.info("Mapping M_EMS_SSAL2_TRAN_EMS_PHA_QPS_CHK completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_EMS_SSAL2_TRAN_EMS_PHA_QPS_CHK FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_EMS_SSAL2_TRAN_EMS_PHA_QPS_CHK", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_EMS_SSAL2_TRAN_EMS_PHA_QPS_CHK")
