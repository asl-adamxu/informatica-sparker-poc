'''
* Script - M_EMS_SSAL2_TRAN_EMS_REF_MKT_RENT_CATG.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None,
    ...
    """
    Execute the M_EMS_SSAL2_TRAN_EMS_REF_MKT_RENT_CATG mapping transformations.
    ...
    logger.info("Start to run mapping M_EMS_SSAL2_TRAN_EMS_REF_MKT_RENT_CATG")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_EMS_SSAL2_TRAN_EMS_REF_MKT_RENT_CATG", config)
    ...
    try:
        logger.info("Step: apply_SQ_EMS_REF_MKT_RENT_CATG")
        # Source Qualifier: apply_SQ_EMS_REF_MKT_RENT_CATG
        # SQL Pushdown - executes Informatica SQ SQL on source database
        _conn = lib.get_db_config(config, "SSA")
        # Parameterize schema from connection config
        _schema = _conn.get("schema", "") or "SSA"
        query = f"""SELECT MKT_RENT_CATG_CODE as MKT_RENT_CATG_CODE, MKT_RENT_CATG_DESP as MKT_RENT_CATG_DESP, MKT_RENT_SNSH_MTH as MKT_RENT_SNSH_MTH, REF_CODE_BGN_DATE as REF_CODE_BGN_DATE, REF_CODE_END_DATE as REF_CODE_END_DATE, LAST_REC_TXN_TYPE_CODE as LAST_REC_TXN_TYPE_CODE, LAST_REC_TXN_DATE as LAST_REC_TXN_DATE, LAST_REC_TXN_USER_ID as LAST_REC_TXN_USER_ID 

FROM EMS_REF_MKT_RENT_CATG"""
        query = query.replace("$$v_snsh_date", v_snsh_date)
        query = query.replace("$$v_init_flag", v_init_flag)
        df_SQ_EMS_REF_MKT_RENT_CATG = lib.read_sql(spark, _conn, query=query)
        # Rename SQL result columns to SQ output ports 
        # name match first, then positional fallback (handles unaliased expressions)
        _sql_cols = df_SQ_EMS_REF_MKT_RENT_CATG.columns
        _port_cols = ["MKT_RENT_CATG_CODE", "MKT_RENT_CATG_DESP", "MKT_RENT_SNSH_MTH", "REF_CODE_BGN_DATE", "REF_CODE_END_DATE", "LAST_REC_TXN_TYPE_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_USER_ID"]
        _rename_map = {}
        _used_ports = set()
        ...
        for _sc in _sql_cols:
            ...
            if _pi < len(_port_cols):
                ...
                _used_ports.add(_pi)
                _pi += 1
        df_SQ_EMS_REF_MKT_RENT_CATG = df_SQ_EMS_REF_MKT_RENT_CATG.select(*[col(f"`{old}`").alias(new) for old, new in _rename_map.items()])
        # Select only SQ output ports (matches Informatica behavior)
        # ports the SQL didn't return become lit(None) so downstream references never fail
        df_SQ_EMS_REF_MKT_RENT_CATG = df_SQ_EMS_REF_MKT_RENT_CATG.select([col(c) if c.lower() in [x.lower() for x in df_SQ_EMS_REF_MKT_RENT_CATG.columns] else lit(None).alias(c) for c in _port_cols])
        
        ctx.register_df("df_SQ_EMS_REF_MKT_RENT_CATG", df_SQ_EMS_REF_MKT_RENT_CATG)
        
        logger.info("Step: apply_EXP_BK")
        # Expression: apply_EXP_BK
        df_EXP_BK = df_SQ_EMS_REF_MKT_RENT_CATG
        df_EXP_BK = df_EXP_BK.withColumn("DUMMY_DATE", expr("NULL"))
        # Ensure any missing pass-through columns exist (no connector feeding them)
        for _col in ["MKT_RENT_CATG_CODE", "MKT_RENT_CATG_DESP", "MKT_RENT_SNSH_MTH", "REF_CODE_BGN_DATE", "REF_CODE_END_DATE", "LAST_REC_TXN_TYPE_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_USER_ID"]:
        ...
        _conn = lib.get_db_config(config, "SSA")
        df_DLKP_SOR_MSTR = lib.read_sql(spark, _conn, table="SOR_EMS_REF_MKT_RENT_CATG")
        ...
        df_lkp_merge_EXP_BK = lib.dynamic_lookup(
            ...
            name='DLKP_SOR_MSTR',
            join_predicates=[{'source_col': 'IN_MKT_RENT_CATG_CODE', 'lookup_col': 'MKT_RENT_CATG_CODE'}],
            output_columns=['MKT_RENT_CATG_KEY', 'MKT_RENT_CATG_CODE'],
            lookup_output_fields=[
                {'name': 'MKT_RENT_CATG_KEY', 'ref_field': 'Sequence-Id', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'integer'},
                {'name': 'MKT_RENT_CATG_CODE', 'ref_field': 'MKT_RENT_CATG_CODE', 'ignore_in_compare': True, 'ignore_null_inputs': False, 'datatype': 'string'}
            ],
            new_lookup_row_col='NewLookupRow',
            sequence_config={'output_col': 'MKT_RENT_CATG_KEY'},
...
where SOR_EMS_REF_MKT_RENT_CATG_STS.MKT_RENT_CATG_KEY = ss.MKT_RENT_CATG_KEY and SOR_EMS_REF_MKT_RENT_CATG_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_EMS_REF_MKT_RENT_CATG_STS.MKT_RENT_CATG_KEY  """
        ...
        _lkp_input = _lkp_input.withColumn("IN_MKT_RENT_CATG_DESP", col("MKT_RENT_CATG_DESP"))
        _lkp_input = _lkp_input.withColumn("IN_MKT_RENT_SNSH_MTH", col("MKT_RENT_SNSH_MTH"))
        ...
        _lkp_input = _lkp_input.withColumn("IN_MKT_RENT_CATG_KEY", col("MKT_RENT_CATG_KEY"))
        df_lkp_merge_EXP_BK = lib.dynamic_lookup(
            spark=spark,
            input_df=_lkp_input,
            lookup_df=df_DLKP_SOR_STS,
            name='DLKP_SOR_STS',
            join_predicates=[{'source_col': 'IN_MKT_RENT_CATG_KEY', 'lookup_col': 'MKT_RENT_CATG_KEY'}],
            output_columns=['MKT_RENT_CATG_KEY', 'BGN_DATE', 'END_DATE', 'MKT_RENT_CATG_DESP', 'MKT_RENT_SNSH_MTH', 'REF_CODE_BGN_DATE', 'REF_CODE_END_DATE'],
            lookup_output_fields=[
                {'name': 'MKT_RENT_CATG_KEY', 'ref_field': 'MKT_RENT_CATG_KEY', 'ignore_in_compare': True, 'ignore_null_inputs': False, 'datatype': 'integer'},
                ...
                {'name': 'MKT_RENT_CATG_DESP', 'ref_field': 'MKT_RENT_CATG_DESP', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'string'},
                {'name': 'MKT_RENT_SNSH_MTH', 'ref_field': 'MKT_RENT_SNSH_MTH', 'ignore_in_compare': False, 'ignore_null_inputs': False, 'datatype': 'decimal'},
        ...
        df_EXPTRANS_MSTR = df_EXPTRANS_MSTR.withColumn("v_SSAL2_TBL_NAME", expr("'SSA_EMS_REF_MKT_RENT_CATG'"))
        ...
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "SSA")
        query = f"""SELECT MKT_RENT_CATG_KEY as SURROGATE_KEY, NULL as DUMMY  FROM SSA_EMS_REF_MKT_RENT_CATG"""
        query = query.replace("$$v_snsh_date", v_snsh_date)
        query = query.replace("$$v_init_flag", v_init_flag)
        ...
        _lkp_input = _lkp_input.withColumn("IN_SURROGATE_KEY", col("MKT_RENT_CATG_KEY"))
        ...
        df_lkp_merge_FILTRANS_MSTR = lib.dynamic_lookup(
            ...
            lookup_output_fields=[
                {'name': 'SURROGATE_KEY', 'ref_field': 'MKT_RENT_CATG_KEY', 'ignore_in_compare': True, 'ignore_null_inputs': False, 'datatype': 'integer'},
        ...
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "SSA")
        query = f"""SELECT MKT_RENT_CATG_KEY as SURROGATE_KEY, NULL as DUMMY  FROM SSA_EMS_REF_MKT_RENT_CATG_STS"""
        query = query.replace("$$v_snsh_date", v_snsh_date)
        query = query.replace("$$v_init_flag", v_init_flag)
        ...
        logger.info("Step: write_SSA_EMS_REF_MKT_RENT_CATG")
        # Write to Target: write_SSA_EMS_REF_MKT_RENT_CATG
        df_write = df_UPD_SSAL2_MSTR
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"AGMT_IND": "OUT_AGMT_IND", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "MKT_RENT_CATG_CODE": "MKT_RENT_CATG_CODE", "MKT_RENT_CATG_KEY": "MKT_RENT_CATG_KEY", "OPR_IND": "OUT_V_OPR_IND"}
        ...
        _del_key_cols = ['mkt_rent_catg_key', 'mkt_rent_catg_code', 'agmt_ind', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            ...
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_EMS_REF_MKT_RENT_CATG", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['mkt_rent_catg_key', 'mkt_rent_catg_code', 'agmt_ind', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
            ...
            if _upd_set_cols:
                ...
                lib.batch_update(spark, conn_target, "SSA_EMS_REF_MKT_RENT_CATG", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        ...
        _target_cols = ['MKT_RENT_CATG_KEY', 'MKT_RENT_CATG_CODE', 'AGMT_IND', 'LAST_REC_TXN_TYPE_CODE', 'LAST_REC_TXN_DATE', 'OPR_IND', 'SOR_DATE']
        df_write = df_write.select(*[col for col in _target_cols if col.lower() in [x.lower() for x in df_write.columns]])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "SSA_EMS_REF_MKT_RENT_CATG", mode="append")

        logger.info("write_SSA_EMS_REF_MKT_RENT_CATG write completed")
        logger.info("Step: write_SSA_EMS_REF_MKT_RENT_CATG_STS")
        # Write to Target: write_SSA_EMS_REF_MKT_RENT_CATG_STS
        df_write = df_UPD_SSAL2_STS
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"BGN_DATE": "OUT_V_BGN_DATE", "END_DATE": "OUT_V_END_DATE", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "MKT_RENT_CATG_DESP": "MKT_RENT_CATG_DESP", "MKT_RENT_CATG_KEY": "MKT_RENT_CATG_KEY", "MKT_RENT_SNSH_MTH": "MKT_RENT_SNSH_MTH", "OPR_IND": "OUT_V_OPR_IND", "REF_CODE_BGN_DATE": "REF_CODE_BGN_DATE", "REF_CODE_END_DATE": "REF_CODE_END_DATE", "SOR_DATE": "OUT_SOR_DATE"}
        for _tgt_col, _src_col in _field_map.items():
            if _tgt_col.lower() not in [x.lower() for x in df_write.columns] and _src_col.lower() in [x.lower() for x in df_write.columns]:
                # Drop any column that would conflict case-insensitively with the target name 
                for _c in list(df_write.columns):
        ...
        df_write = df_write.drop("_update_flag")
        # DELETE: composite key batch delete (all key columns together)
        _del_key_cols = ['mkt_rent_catg_key', 'bgn_date', 'end_date', 'mkt_rent_catg_desp', 'mkt_rent_snsh_mth', 'ref_code_bgn_date', 'ref_code_end_date', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            _del_rows = [tuple(r[c] for c in _del_key_cols) for r in _df_del.select(*_del_key_cols).distinct().collect()]
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_EMS_REF_MKT_RENT_CATG_STS", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['mkt_rent_catg_key', 'bgn_date', 'end_date', 'mkt_rent_catg_desp', 'mkt_rent_snsh_mth', 'ref_code_bgn_date', 'ref_code_end_date', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
            _upd_set_cols = [c for c in _df_upd.columns if c.lower() not in [k.lower() for k in _upd_key_cols]]
            if _upd_set_cols:
                _upd_rows = [tuple(r[c] for c in _upd_set_cols + _upd_key_cols) for r in _df_upd.collect()]
                lib.batch_update(spark, conn_target, "SSA_EMS_REF_MKT_RENT_CATG_STS", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        # INSERT — set df_write to _df_ins so it flows through normal write path
        df_write = _df_ins
        # Select only target-defined columns (field_map already handled name alignment)
        _target_cols = ['MKT_RENT_CATG_KEY', 'BGN_DATE', 'END_DATE', 'MKT_RENT_CATG_DESP', 'MKT_RENT_SNSH_MTH', 'REF_CODE_BGN_DATE', 'REF_CODE_END_DATE', 'LAST_REC_TXN_TYPE_CODE', 'LAST_REC_TXN_DATE', 'OPR_IND', 'SOR_DATE']
        df_write = df_write.select(*[col for col in _target_cols if col.lower() in [x.lower() for x in df_write.columns]])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "SSA_EMS_REF_MKT_RENT_CATG_STS", mode="append")

        logger.info("write_SSA_EMS_REF_MKT_RENT_CATG_STS write completed")
        
        metrics.complete()
        logger.info("Mapping M_EMS_SSAL2_TRAN_EMS_REF_MKT_RENT_CATG completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_EMS_SSAL2_TRAN_EMS_REF_MKT_RENT_CATG FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_EMS_SSAL2_TRAN_EMS_REF_MKT_RENT_CATG", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_EMS_SSAL2_TRAN_EMS_REF_MKT_RENT_CATG")
