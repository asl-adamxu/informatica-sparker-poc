...
SELECT NHS_HOS_CRT.HSE_DSTR_KEY FROM NHS_HOS_CRT"""
        ...
        _dup_cnt = df_MPLT_AGMT_NHS_HSE_DSTR_LKP_DYN_SOR_NHS_HSE_DSTR.groupBy(col("NHS_HSE_DSTR_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
            raise RuntimeError(f"Lookup apply_MPLT_AGMT_NHS_HSE_DSTR_LKP_DYN_SOR_NHS_HSE_DSTR: {_dup_cnt} duplicate keys found — Report Error policy")
        ...
        df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_NULL_BKEY = _lkp_input.alias("_main").join(
        ...
        if _nlr_lkp_cols:
            df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_NULL_BKEY = df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_NULL_BKEY.withColumn("NewLookupRow_LKP_DYN_SOR_NHS_HSE_DSTR", expr("CASE WHEN `" + _nlr_lkp_cols[0] + "` IS NULL THEN 0 ELSE 1 END"))
        else:
            df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_NULL_BKEY = df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_NULL_BKEY.withColumn("NewLookupRow_LKP_DYN_SOR_NHS_HSE_DSTR", lit(1))
        ...
        # Report Error on multiple match: check for duplicate join keys
        _dup_cnt = df_MPLT_AGMT_NHS_HSE_DSTR_LKP_DYN_SSA_NHS_HSE_DSTR.groupBy(col("SURROGATE_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
            raise RuntimeError(f"Lookup apply_MPLT_AGMT_NHS_HSE_DSTR_LKP_DYN_SSA_NHS_HSE_DSTR: {_dup_cnt} duplicate keys found — Report Error policy")
        # Rename upstream columns to match lookup input port names before join
        _lkp_input = df_MPLT_AGMT_NHS_HSE_DSTR_EXP_DUMMY
        # Join condition: HSE_DSTR_KEY=SURROGATE_KEY
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_DUMMY = _lkp_input.alias("_main").join(
        ...
        if (_nlr_key.lower() not in [x.lower() for x in _lkp_input.columns]
            ...
            _nlr_lkp_cols.insert(0, _nlr_key)
        if _nlr_lkp_cols:
            df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_DUMMY = df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_DUMMY.withColumn("NewLookupRow_LKP_DYN_SSA_NHS_HSE_DSTR", expr("CASE WHEN `" + _nlr_lkp_cols[0] + "` IS NULL THEN 0 ELSE 1 END"))
        else:
            df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_DUMMY = df_mplt_lkp_chain_MPLT_AGMT_NHS_HSE_DSTR_EXP_DUMMY.withColumn("NewLookupRow_LKP_DYN_SSA_NHS_HSE_DSTR", lit(1))
