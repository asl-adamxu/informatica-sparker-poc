...
FROM NHS_EST_BANK_ITEM"""
        ...
        df_MPLT_AGMT_NHS_RVN_CLCT_TRML_input = df_EXP_BK
        ...
        _cc = list(dict.fromkeys(c for c in df_EXP_BK.columns if c.lower() in [x.lower() for x in df_MPLT_AGMT_NHS_EST_BANK.columns]))
        if _cc:
            __lkp_dup = [c for c in df_MPLT_AGMT_NHS_EST_BANK.columns if c.lower() in [x.lower() for x in df_EXP_BK.columns] and c.lower() not in [x.lower() for x in _cc]]
            ...
            logger.warning("No common columns between df_EXP_BK and df_MPLT_AGMT_NHS_EST_BANK — using synthetic key join")
...
where SOR_NHS_EST_BANK_ITEM_STS.EST_BANK_ITEM_KEY = ss.EST_BANK_ITEM_KEY and SOR_NHS_EST_BANK_ITEM_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_NHS_EST_BANK_ITEM_STS.EST_BANK_ITEM_KEY  """
        ...
        # Merge on common columns — drop lookup columns that duplicate non-key input columns. 
        # Matches are CASE-INSENSITIVE: SQ ports may be lowercase while Oracle lookup
        _cc = list(dict.fromkeys(c for c in df_lkp_merge_EXP_BK.columns if c.lower() in [x.lower() for x in df_MPLT_AGMT_NHS_RVN_CLCT_TRML.columns]))
        if _cc:
            __lkp_dup = [c for c in df_MPLT_AGMT_NHS_RVN_CLCT_TRML.columns if c.lower() in [x.lower() for x in df_lkp_merge_EXP_BK.columns] and c.lower() not in [x.lower() for x in _cc]]
            # Break attribute lineage on the merged side: when both inputs are built from the same source plan. 
            # Re-projecting with aliases gives this side fresh attribute IDs without changing rows or column names.
            __rhs = df_MPLT_AGMT_NHS_RVN_CLCT_TRML.drop(*__lkp_dup) if __lkp_dup else df_MPLT_AGMT_NHS_RVN_CLCT_TRML
            __rhs = __rhs.select(*[col(c).alias(c) for c in __rhs.columns])
            ...
            logger.warning("No common columns between df_lkp_merge_EXP_BK and df_MPLT_AGMT_NHS_RVN_CLCT_TRML — using synthetic key join")
            __rhs = df_MPLT_AGMT_NHS_RVN_CLCT_TRML.withColumn("_join_key", lit(1))
            __rhs = __rhs.select(*[col(c).alias(c) for c in __rhs.columns])
