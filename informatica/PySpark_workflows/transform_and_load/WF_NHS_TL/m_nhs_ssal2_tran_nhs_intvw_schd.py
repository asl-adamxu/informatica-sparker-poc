...
FROM NHS_INTVW_SCHD"""
        ...
        _cc = list(dict.fromkeys(c for c in df_EXP_BK.columns if c.lower() in [x.lower() for x in df_MPLT_AGMT_NHS_PHASE.columns]))
        if _cc:
            __lkp_dup = [c for c in df_MPLT_AGMT_NHS_PHASE.columns if c.lower() in [x.lower() for x in df_EXP_BK.columns] and c.lower() not in [x.lower() for x in _cc]]
            ...
            logger.warning("No common columns between df_EXP_BK and df_MPLT_AGMT_NHS_PHASE — using synthetic key join")
