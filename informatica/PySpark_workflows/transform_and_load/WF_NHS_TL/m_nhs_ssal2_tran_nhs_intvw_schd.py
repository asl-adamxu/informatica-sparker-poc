'''
* Script - M_NHS_SSAL2_TRAN_NHS_INTVW_SCHD.py
...
def run_mapping(ctx: lib.SparkContext = None, metrics=None, job_params=None,
    ...
    """
    Execute the M_NHS_SSAL2_TRAN_NHS_INTVW_SCHD mapping transformations.
    ...
    logger.info("Start to run mapping M_NHS_SSAL2_TRAN_NHS_INTVW_SCHD")

    config = lib.load_config('env/config.yml')
    objects = config.get('objects', {})

    spark = ctx.spark if ctx else lib.get_spark_session("M_NHS_SSAL2_TRAN_NHS_INTVW_SCHD", config)
...
FROM NHS_INTVW_SCHD"""
        ...
        for _sc in _sql_cols:
            ...
            if _pi < len(_port_cols):
                ...
                _used_ports.add(_pi)
                _pi += 1
        df_SQ_NHS_INTVW_SCHD = df_SQ_NHS_INTVW_SCHD.select(*[col(f"`{old}`").alias(new) for old, new in _rename_map.items()])
        # Select only SQ output ports (matches Informatica behavior)
        # ports the SQL didn't return become lit(None) so downstream references never fail
        df_SQ_NHS_INTVW_SCHD = df_SQ_NHS_INTVW_SCHD.select([col(c) if c.lower() in [x.lower() for x in df_SQ_NHS_INTVW_SCHD.columns] else lit(None).alias(c) for c in _port_cols])
        
        ctx.register_df("df_SQ_NHS_INTVW_SCHD", df_SQ_NHS_INTVW_SCHD)
        
        logger.info("Step: apply_EXP_BK")
        # Expression: apply_EXP_BK
        df_EXP_BK = df_SQ_NHS_INTVW_SCHD
        df_EXP_BK = df_EXP_BK.withColumn("DUMMY_DATE", expr("NULL"))
        # Ensure any missing pass-through columns exist (no connector feeding them)
        for _col in ["PHASE_CODE", "INTVW_SCHD_DATE", "INTVW_SCHD_BGN_TIME", "INTVW_SCHD_END_TIME", "INTVW_SCHD_APLY_MAX_NUM", "INTVW_SCHD_APLY_ALCT_NUM", "ROW_VER_NUM", "LAST_REC_TXN_TYPE_CODE", "LAST_REC_TXN_DATE", "LAST_REC_TXN_USER_ID"]:
        ...
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "SSA")
        df_DLKP_SOR_MSTR = lib.read_sql(spark, _conn, table="SOR_NHS_INTVW_SCHD")
        ...
        _dup_cnt = df_DLKP_SOR_MSTR.groupBy(col("PHASE_KEY"), col("NHS_INTVW_SCHD_DATE"), col("NHS_INTVW_SCHD_BGN_TIME")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
            raise RuntimeError(f"Lookup apply_DLKP_SOR_MSTR: {_dup_cnt} duplicate keys found — Report Error policy")
        # Rename upstream columns to match lookup input port names before join
        _lkp_input = df_MPLT_AGMT_NHS_PHASE
        _lkp_input = _lkp_input.withColumn("IN_NHS_INTVW_SCHD_DATE", col("INTVW_SCHD_DATE"))
        _lkp_input = _lkp_input.withColumn("IN_NHS_INTVW_SCHD_BGN_TIME", col("INTVW_SCHD_BGN_TIME"))
        _lkp_input = _lkp_input.withColumn("IN_PHASE_KEY", col("PHASE_KEY"))
        # Join condition: IN_PHASE_KEY=PHASE_KEY AND IN_NHS_INTVW_SCHD_DATE=NHS_INTVW_SCHD_DATE AND IN_NHS_INTVW_SCHD_BGN_TIME=NHS_INTVW_SCHD_BGN_TIME
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_lkp_merge_7 = _lkp_input.alias("_main").join(
            broadcast(df_DLKP_SOR_MSTR).alias("_lkp"),
            (col("_main.IN_PHASE_KEY") == col("_lkp.PHASE_KEY")) &
            (col("_main.IN_NHS_INTVW_SCHD_DATE") == col("_lkp.NHS_INTVW_SCHD_DATE")) &
            (col("_main.IN_NHS_INTVW_SCHD_BGN_TIME") == col("_lkp.NHS_INTVW_SCHD_BGN_TIME")),
            "left"
        ).select(
            *[_lkp_input[c] for c in _lkp_input.columns],
...
where SOR_NHS_INTVW_SCHD_STS.INTVW_SCHD_KEY = ss.INTVW_SCHD_KEY and SOR_NHS_INTVW_SCHD_STS.bgn_date = ss.max_bgn_date
 ORDER BY SOR_NHS_INTVW_SCHD_STS.INTVW_SCHD_KEY -- """
        ...
        # Report Error on multiple match: check for duplicate join keys
        _dup_cnt = df_DLKP_SOR_STS.groupBy(col("INTVW_SCHD_KEY")).count().filter(col("count") > 1).count()
        if _dup_cnt > 0:
        ...
        _lkp_input = _lkp_input.withColumn("IN_INTVW_SCHD_END_TIME", col("INTVW_SCHD_END_TIME"))
        _lkp_input = _lkp_input.withColumn("IN_INTVW_SCHD_APLY_MAX_NUM", col("INTVW_SCHD_APLY_MAX_NUM"))
        _lkp_input = _lkp_input.withColumn("IN_INTVW_SCHD_APLY_ALCT_NUM", col("INTVW_SCHD_APLY_ALCT_NUM"))
        ...
        # Join condition: IN_INTVW_SCHD_KEY=INTVW_SCHD_KEY
        # Alias-based join: _main.<source_col> == _lkp.<lookup_col>
        df_lkp_merge_7 = _lkp_input.alias("_main").join(
            broadcast(df_DLKP_SOR_STS).alias("_lkp"),
            (col("_main.IN_INTVW_SCHD_KEY") == col("_lkp.INTVW_SCHD_KEY")),
            "left"
        ).select(
            *[_lkp_input[c] for c in _lkp_input.columns],
        ...
        df_EXPTRANS_MSTR = df_EXPTRANS_MSTR.withColumn("v_SSAL2_TBL_NAME", expr("'SSA_NHS_INTVW_SCHD'"))
        ...
        _lkp_input = _lkp_input.withColumn("IN_SURROGATE_KEY", col("INTVW_SCHD_KEY"))
        ...
        # Resolve connection by alias (supports lookup/source connections dynamically)
        _conn = lib.get_db_config(config, "SSA")
        query = f"""SELECT INTVW_SCHD_KEY as SURROGATE_KEY, NULL as DUMMY  FROM SSA_NHS_INTVW_SCHD_STS"""
        query = query.replace("$$v_snsh_date", v_snsh_date)
        query = query.replace("$$v_init_flag", v_init_flag)
        ...
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"AGMT_IND": "OUT_AGMT_IND", "INTVW_SCHD_KEY": "INTVW_SCHD_KEY", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "NHS_INTVW_SCHD_BGN_TIME": "NHS_INTVW_SCHD_BGN_TIME", "NHS_INTVW_SCHD_DATE": "NHS_INTVW_SCHD_DATE", "OPR_IND": "OUT_V_OPR_IND", "PHASE_KEY": "PHASE_KEY"}
        for _tgt_col, _src_col in _field_map.items():
            if _tgt_col.lower() not in [x.lower() for x in df_write.columns] and _src_col.lower() in [x.lower() for x in df_write.columns]:
                # Drop any column that would conflict case-insensitively with the target name 
                for _c in list(df_write.columns):
        ...
        _del_key_cols = ['intvw_schd_key', 'phase_key', 'nhs_intvw_schd_date', 'nhs_intvw_schd_bgn_time', 'agmt_ind', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            ...
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_NHS_INTVW_SCHD", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['intvw_schd_key', 'phase_key', 'nhs_intvw_schd_date', 'nhs_intvw_schd_bgn_time', 'agmt_ind', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
            ...
            if _upd_set_cols:
                ...
                lib.batch_update(spark, conn_target, "SSA_NHS_INTVW_SCHD", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        ...
        logger.info("Step: write_SSA_NHS_INTVW_SCHD_STS")
        # Write to Target: write_SSA_NHS_INTVW_SCHD_STS
        df_write = df_UPD_SSAL2_STS
        # Map source columns to target columns using connector field map (handles name
        # mismatches) — done BEFORE the _update_flag split so UPDATE/DELETE use target
        # column names in batch_update/batch_delete.
        _field_map = {"BGN_DATE": "OUT_V_BGN_DATE", "END_DATE": "OUT_V_END_DATE", "INTVW_SCHD_APLY_ALCT_NUM": "INTVW_SCHD_APLY_ALCT_NUM", "INTVW_SCHD_APLY_MAX_NUM": "INTVW_SCHD_APLY_MAX_NUM", "INTVW_SCHD_END_TIME": "INTVW_SCHD_END_TIME", "INTVW_SCHD_KEY": "INTVW_SCHD_KEY", "LAST_REC_TXN_DATE": "OUT_V_LAST_REC_TXN_DATE", "LAST_REC_TXN_TYPE_CODE": "OUT_V_LAST_REC_TXN_TYPE_CODE", "OPR_IND": "OUT_V_OPR_IND", "ROW_VER_NUM": "ROW_VER_NUM", "SOR_DATE": "OUT_SOR_DATE"}
        for _tgt_col, _src_col in _field_map.items():
            if _tgt_col.lower() not in [x.lower() for x in df_write.columns] and _src_col.lower() in [x.lower() for x in df_write.columns]:
                # Drop any column that would conflict case-insensitively with the target name 
                for _c in list(df_write.columns):
        ...
        df_write = df_write.drop("_update_flag")
        # DELETE: composite key batch delete (all key columns together)
        _del_key_cols = ['intvw_schd_key', 'bgn_date', 'end_date', 'intvw_schd_end_time', 'intvw_schd_aply_max_num', 'intvw_schd_aply_alct_num', 'row_ver_num', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
        if not _df_del.rdd.isEmpty():
            _del_rows = [tuple(r[c] for c in _del_key_cols) for r in _df_del.select(*_del_key_cols).distinct().collect()]
            if _del_rows:
                lib.batch_delete_composite(spark, conn_target, "SSA_NHS_INTVW_SCHD_STS", _del_key_cols, _del_rows, 1000)
        # UPDATE: batch update via JDBC
        if not _df_upd.rdd.isEmpty():
            _upd_key_cols = ['intvw_schd_key', 'bgn_date', 'end_date', 'intvw_schd_end_time', 'intvw_schd_aply_max_num', 'intvw_schd_aply_alct_num', 'row_ver_num', 'last_rec_txn_type_code', 'last_rec_txn_date', 'opr_ind', 'sor_date']
            _upd_set_cols = [c for c in _df_upd.columns if c.lower() not in [k.lower() for k in _upd_key_cols]]
            if _upd_set_cols:
                _upd_rows = [tuple(r[c] for c in _upd_set_cols + _upd_key_cols) for r in _df_upd.collect()]
                lib.batch_update(spark, conn_target, "SSA_NHS_INTVW_SCHD_STS", _upd_set_cols, _upd_key_cols, _upd_rows, 1000)
        # INSERT — set df_write to _df_ins so it flows through normal write path
        df_write = _df_ins
        # Select only target-defined columns (field_map already handled name alignment)
        _target_cols = ['INTVW_SCHD_KEY', 'BGN_DATE', 'END_DATE', 'INTVW_SCHD_END_TIME', 'INTVW_SCHD_APLY_MAX_NUM', 'INTVW_SCHD_APLY_ALCT_NUM', 'ROW_VER_NUM', 'LAST_REC_TXN_TYPE_CODE', 'LAST_REC_TXN_DATE', 'OPR_IND', 'SOR_DATE']
        df_write = df_write.select(*[col for col in _target_cols if col.lower() in [x.lower() for x in df_write.columns]])
        # Write to database table (Oracle, etc.) using write_table (supports smart repartition, batch size, empty-df skip)
        lib.write_table(df_write, conn_target, "SSA_NHS_INTVW_SCHD_STS", mode="append")

        logger.info("write_SSA_NHS_INTVW_SCHD_STS write completed")
        
        metrics.complete()
        logger.info("Mapping M_NHS_SSAL2_TRAN_NHS_INTVW_SCHD completed: SUCCESS")
        return True
        
    except Exception as e:
        metrics.fail(e)
        logger.error(f"Mapping M_NHS_SSAL2_TRAN_NHS_INTVW_SCHD FAILED: {e}", exc_info=True)
        return False
...
def main():
    ...
    logger = lib.get_logger()
    config = lib.load_config('env/config.yml')
    spark = lib.get_spark_session("M_NHS_SSAL2_TRAN_NHS_INTVW_SCHD", config)
    ctx = lib.SparkContext(spark)
    metrics = lib.MappingMetrics("M_NHS_SSAL2_TRAN_NHS_INTVW_SCHD")
