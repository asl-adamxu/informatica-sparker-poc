...
**Version v2026.08.10** — A Python framework that converts Informatica PowerCenter workflow/mapping XML exports into PySpark code deployable to Databricks or YARN Spark clusters. **Tested against Informatica output — data results match.**

**v2026.08.10 highlights (transform & load workflow WF_NHS_TL)**: all 174 mappings runtime-verified; exact Dynamic Lookup cache conversion via `applyInPandas` (equivalent RDD fallback) with `NewLookupRow` 0/1/2 semantics, base duplicate-key Report Error / CMN_1650, and global Sequence-Id pre-allocation; lookup-fed FILTRANS / multi-upstream lookup wiring corrected; mapplet rename and mapplet lookup multiple-match policy; `DECODE(x, NULL, ...)` NULL semantics; stable semantic DataFrame naming with empty mapplet rename steps removed.
...
- **Dynamic Lookup is now converted exactly** for the `Report Error` policy used by WF_NHS_TL and WF_EMS_TL: base duplicate condition keys fail the session (CMN_1650), duplicate keys in the *input* stream are legal and classified by the cache state machine (0/1/2).
- **Remaining limits**: `Synchronize Dynamic Cache=YES` and cross-session persistent cache reuse are not emulated (the cache is seeded from the lookup source on every run); `Update Else Insert=YES` / non-insert row types are not supported; EMS's `Use First Value` dynamic-lookup variant is not yet converted. These are scheduled for the WF_EMS_TL round.
