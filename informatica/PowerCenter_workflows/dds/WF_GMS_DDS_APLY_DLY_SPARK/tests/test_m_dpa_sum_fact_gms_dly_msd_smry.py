...
def spark():
    spark = SparkSession.builder.master("local[2]").appName("pytest-m_dpa_sum_fact_gms_dly_msd_smry").getOrCreate()
    yield spark
    spark.stop()
...
def test_run_mapping_basic(spark, monkeypatch):
    ...
    assert res is True
    assert "DPA_FACT_GMS_DLY_MSD_SMRY" in captured
