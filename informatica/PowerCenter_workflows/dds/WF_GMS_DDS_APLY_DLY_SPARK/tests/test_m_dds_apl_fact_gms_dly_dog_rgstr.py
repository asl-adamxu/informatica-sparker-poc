...
def spark():
    spark = SparkSession.builder.master("local[2]").appName("pytest-m_dds_apl_fact_gms_dly_dog_rgstr").getOrCreate()
    yield spark
    spark.stop()
...
def test_run_mapping_basic(spark, monkeypatch):
    ...
    import m_dds_apl_fact_gms_dly_dog_rgstr as mod

    # fake read_sql depending on table/query
    ...
    assert res is True
    assert "DDS_FACT_GMS_DLY_DOG_RGSTR" in captured
