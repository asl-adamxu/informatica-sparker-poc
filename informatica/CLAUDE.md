...
### E2E Test Generation + Validation Framework Removed (v2026.08.18)

- **Removed from the repo**: `validation/` package (the `validate` CLI, comparator/loader/runner/report — row-count/hash comparison against the Informatica DB), `informatica_sparker/metadata/` (`metadata.json` / `manifest.json` export + `build-manifest`), `informatica_sparker/test_generator.py` + `convert --with-tests` + `templates/test/`, and their tests (`tests/test_validation.py`, `tests/test_metadata_builder.py`, 57 tests).
- **Why**: E2E test generation moved OUT of this repository — the E2E pytest suites in `tests/` are generated and maintained elsewhere. Generated-code validation is now done through those external suites, not the in-repo validation framework.
- **CLI now**: only `convert` + `analyze` (no `build-manifest`, `validate`, `--with-tests`). `service.py` no longer writes `metadata.json` or `tests/`; `config.yml.j2` no longer emits the `validation:` section; `pyproject.toml` packages.find is `["informatica_sparker*"]` only.
- **Tests**: full suite now **139 passed** (was 196; the 57 removed tests covered the deleted modules).
- **Rebuild note**: wheel rebuilt + reinstalled (`python3.11 -m build` + `pip install --user --force-reinstall dist/informatica_sparker-2026.8.18-*.whl`); CLI verified from site-packages.
...
- **Fix (generator)**: shared module-level `_classify_update_strategy(strategy_expr)` used by BOTH the Update Strategy handler and the Write Target handler, so they can never disagree. Numeric literals normalize to their Informatica constants before classification: **0=DD_INSERT, 1=DD_UPDATE, 2=DD_DELETE, 3=DD_REJECT** (user-verified; NOT DD_UPSERT — no reject-row support, falls to insert). Field-ref detection requires an identifier shape (`_BARE_IDENTIFIER_RE`). Test: `test_numeric_update_strategy_expression.py` (synthetic XML: 0/1/2/field-ref); 191 tests pass.
- **Fix (runtime)**: `lib.update_strategy`'s dynamic `strategy_field` branch now dual-compares each value against the DD_* string constant AND its numeric equivalent (`col == "DD_UPDATE" | col == 1` → "U") — a strategy FIELD carrying raw numerics (0/1/2/3) works like the string-constant domain. Mixed comparisons are safe under Spark type coercion (non-castable literal → NULL → branch skipped; a string `'0'` matches `== 0` too). `3`/`DD_REJECT` → flag `"R"`, which the write target's I/U/D split drops (rejected rows never reach the target); unknown values keep the otherwise→"I" fallback. Tests: `test_dynamic_strategy_numeric_flag`, `test_dynamic_strategy_mixed_string_and_numeric`; 193 tests pass. The `decode(IN_DLK_SOR_CACHE_STATUS, 1, DD_INSERT, ...)` mapplet fields keep string-constant values (expr_translator renders `'DD_INSERT'`), so both domains coexist.
- **Regenerated**: WF_EMS_TL reconverted — 581 mappings, 0 warnings / 0 errors; `# Strategy: DD_INSERT` replaces `# Strategy: 0`, and the previously failing write step renders a plain append (no `has_update_flag`/`delete_keys`).
...
- **Update Strategy semantics recap**: XML `Update Strategy Expression` (e.g. `UPDATE_FLAG`) with `DD_INSERT`/`DD_UPDATE`/`DD_DELETE` constants → generator renders the constants as **strings** (`'DD_INSERT'`) in the computed column → `lib.update_strategy` compares (string constants AND numeric 0/1/2/3 equivalents, dual-check) to build `_update_flag` ('I'/'U'/'D'/'R', otherwise→'I') → `lib.write_target` splits: batch delete by keys → batch update by keys (target columns only) → INSERT rows flow to normal append. Matches Informatica per-row 0/1/2 semantics for this convention; caveats: `DD_REJECT` (3) rows are flagged "R" and dropped by the split (no reject-file support), and numeric literal expressions (0/1/2) normalize to their DD_* constants at generation (see Numeric Update Strategy Expressions above).
...
As of **2026-08-18** (version **v2026.08.18**), **all workflows are converted and runtime-validated** — every completed conversion run reports **0 warnings / 0 errors**. The final round was **WF_EMS_TL (581 mappings, 585 workflow steps)**: reconverted and fully runtime-verified (SUCCESS, 0 failures). Fixes in that round: numeric Update Strategy expressions (0/1/2/3 = DD_INSERT/DD_UPDATE/DD_DELETE/DD_REJECT) with shared `_classify_update_strategy` + numeric strategy fields dual-compared in `lib.update_strategy`; trailing-whitespace mapping/session names stripped at parse time; mapplet-internal unconnected INPUT → NULL; update_strategy connector renames; `sq_output` "date/time" → TimestampType; dynamic lookup base-hit compare-and-update (`Output Old Value On Update=YES`).
...
| WF_EMS_TL | 581 | 36M | **Round complete (v2026.08.18)** — all mappings runtime verified (585 workflow steps, 0 failures); numeric Update Strategy (0/1/2/3 + numeric strategy fields), trailing-whitespace names, mapplet-internal unconnected INPUT, update_strategy connector renames, `date/time` port cast, dynamic lookup `Output Old Value On Update=YES` |
